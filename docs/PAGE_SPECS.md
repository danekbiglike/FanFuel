...
- Мобильный первый экран маркетплейса: короткий заголовок, поиск, быстрые категории, CTA и начало товарной ленты.
- Кликабельные объясняющие плашки: `Safe deal`, промокоды авторов, проверенные продавцы.
- Горизонтальная лента "Популярно сейчас" из реальных товаров API.
- Блок "Подборки авторов" с авторскими лентами и переходами в существующий public creator route.
- Блоки быстрых категорий: "Для стримеров" и "Игровые услуги".
- Блок доверия "Почему покупать на FanFuel удобно".
- Нижняя лента "Новые товары" из текущего product list.
- Empty/search no results.
- Legal/category warning для restricted категорий.

### Основные действия пользователя

Искать, фильтровать, открыть товар, применить category/query filter, открыть товарную ленту или подборку автора. Гость и авторизованный пользователь могут отправить запрос из верхней панели, не открывая сначала каталог.
...
Первый экран показывает короткий заголовок, поиск, быстрые категории и начало "Популярно сейчас". Горизонтальные ленты листаются вправо; нижняя лента может быть в две карточки на ширину.

#### Desktop

Hero в две колонки, shelves горизонтальные, нижняя лента плотной сеткой.

### UX-правила

Карточка товара показывает квадратный превью-блок, цену, продавца, рейтинг/нового продавца и safe deal. Не перегружать бейджами. Партнёрская/авторская поддержка показывается отдельным бейджем только при наличии данных `affiliate_percent_bps`.
...
Реализованы каталог `/marketplace` и такая же главная на `/`, загрузка категорий/товаров из API, hero без внутреннего блока поиска, поиск в верхней панели для гостя/авторизованного пользователя, быстрые категории, витринная карточка товара, компактный блок ролей, горизонтальные товарные ленты, авторские подборки, блок доверия, состояния загрузки/пустого списка/ошибки.

### Что нужно изменить

Провести screenshot QA для мобильной версии и light/dark тем; позже улучшить мобильные фильтры как drawer, добавить настоящий route/list для авторов только после обновления IA и расширить category routes после уточнения spec.
