...
- Не использовать в UI звездочки и стрелки: Unicode-символы, эмодзи и декоративные пиктограммы этих форм могут некорректно отображаться на iPhone и выглядят как шаблонное оформление. Для рейтинга, перехода или направления выбирать другой понятный способ отображения; если нужна навигационная иконка, использовать существующую иконку проекта без формы стрелки.
- Освободившееся место не оставлять случайной пустотой: в публичных экранах ритм создают типографика, неровное карандашное подчёркивание, отдельный контурный рисунок и пауза вокруг важного содержимого. Один заметный жест на смысловой блок; не повторять одинаковые штрихи механически.
- Иллюстрация без медиа должна быть связана с видом товара и честно подписана как заглушка. Нельзя подменять обложку выдуманным товарным изображением или дублировать крупным текстом название внутри карточки.
- Избегать декоративных эмодзи и символов, которые не помогают понять действие или состояние.
...
- `/` и `/marketplace` используют одну marketplace-first структуру.
- Покупательский режим выбран по умолчанию независимо от ролей аккаунта; автор и продавец доступны вторичными вкладками.
- Первый viewport должен содержать торговую функцию: поиск/путь к поиску, категории или реальные товары.
- Общий каталог не оформлять как каталог только для стримеров/авторов; creator/OBS assets — одно из направлений цифровых товаров.
- В покупательский режим не возвращать большой platform explainer, role-first hero, длинный набор промо-секций и общий финальный CTA.
- Изменения главной должны проходить regression checklist `UX-TASK-036` и явный product review.

## Маскот и живые реакции

- До `UX-TASK-038` маскот не добавляется в production как сквозной компонент.
- Любая анимация имеет static/reduced-motion fallback и не блокирует основное действие.
- В financial, dispute, moderation и account-blocked состояниях не использовать шутливые или обвиняющие реакции.
- Маскот не должен давить на покупку, донат или ежедневное возвращение.
