...
## 2026-09-24 — пустота после удаления декоративных символов

После удаления звездочек и стрелок главная и тестовая площадка потеряли часть визуального ритма: категории, товарные слоты, пустые состояния и презентационные блоки выглядели незавершёнными. По запросу владельца добавлены неровные карандашные штрихи, разные контурные рисунки по типам товара и спокойные акценты в презентациях. Навигация остаётся текстовой; рисованные линии не изображают стрелки. Товары без медиа остаются честно подписанными. Связь: UX-TASK-040, UX-TASK-044.

## 2026-09-24 — символы в интерфейсе после нового правила

После запрета звездочек и стрелок в UI обнаружены видимые символы в витрине, редакторах, старых страницах, игре и тестовой площадке Ойли. Часть стрелок была записана как CSS Unicode escape, а часть звездочек находилась в декоративном SVG. В рамках прямого запроса владельца они удалены или заменены подписанными действиями и простыми формами. Технические названия клавиш для управления с клавиатуры не менялись. Проверка на реальном iPhone ещё не выполнена.

## 2026-09-22 — UX-TASK-040

Обнаружено и исправлено в рамках marketplace-задачи: /marketplace/catalog и /marketplace/category/[slug] были презентационными заглушками, хотя поиск шапки вёл в каталог; /marketplace/categories содержал ссылки на отсутствующие в миграциях alerts/game-services. Теперь каталоговые маршруты используют существующий Product API и реальные активные категории; путь услуг ведёт в digital-services. У Product нет cover/media поля: временно используется явно подписанная SVG-иллюстрация категории. Настоящие медиа требуют отдельной модели/задачи; выдуманные обложки не добавлены.

Исторические Aurora-разделы ниже не являются текущим визуальным контрактом web; утверждённый контракт и проверки — UX-TASK-040.

## Аудит 2026-09-20: дрейф главной от marketplace-first контракта

Затрагивает: `PAGE-HOME`, `PAGE-MARKETPLACE`, `FLOW-BUYER-HOME`, `FLOW-BUYER-SEARCH`, UX-TASK-029/030.

Найденное расхождение: код уже содержит общий screen `/` и `/marketplace` и переключатель аудиторий, но документация накопила противоречивые требования к hero, creator-commerce презентации, витринам авторов и финальным CTA. При последующих изменениях это позволяет главной снова становиться лендингом либо каталогом преимущественно для стримеров.

Решение для текущей задачи:

- PAGE-HOME и PAGE-MARKETPLACE переведены в `NEEDS_REWORK`;
- UX-TASK-029/030 признаны историческими и заменены UX-TASK-036;
- закреплено: обычный покупатель всегда по умолчанию, общий каталог цифровых товаров, автор/продавец во вторичных вкладках;
- creator/OBS товары остаются важной категорией, но не определением всего marketplace;
- исправление кода не выполнялось, так как текущий запрос касается переработки плана.

Следующее действие: выполнить только UX-TASK-036, провести regression QA и получить явный product review. После этого переходить к живому визуальному направлению UX-TASK-037.
...
## UX-TASK-041 — исправление расхождения с эскизом, 22.09.2026

Владелец отклонил гладкие SVG-линии вместо графита и шаблонную композицию. Исправлено в рамках явного запроса: настоящий фактурный Ойли 48 KB, локальный Caveat вместо системной имитации почерка, Golos и выразительный масштаб заголовка, компактная полоса категорий, landscape-карточки, лаймовая канистра/CTA, контурная корзина и heart-overlay. Огромная сетка пустых мест заменена одним полноценным empty/error-блоком. Реальные публичные обложки получают устойчивый fallback.

Сравнение с исходными локальными изображениями проведено до редактирования. Реальный localhost проверен в браузере; screenshot QA с тестовым наполнением проверяет композицию главной и Studio. Данные тестов не записаны в публичный каталог. Источники, лицензии и оговорки — DESIGN_ASSETS.md. Product review владельца ещё не получен.

Вне этой задачи остаются старые долги /me/profile и полномасштабный seller product-media upload. Текущая поддержка обложек читает существующую публичную product_media. Перед открытием risky categories нужны канонические проверки внешних ID и раздельные региональные ограничения.

## UX-TASK-042 — точечная повторная проверка владельцем, 22.09.2026

Четыре прямоугольные карточки с абстрактными лозунгами не соответствовали marketplace-first направлению; гостевые вкладки выглядели как стандартный underlined tab bar; Ойли дублировался в пустых состояниях и появлялся отдельной секцией внизу. В прямом scope владельца заменены только эти блоки и навигационная таксономия. Теперь каталог представлен конкретными направлениями и линиями, вкладка — SVG-карандашным контуром, на главной — один перемещаемый actor. Исходные замечания и acceptance: UX-TASK-042. Визуальный product review владельца остаётся необходим.
