...
- Аккаунты, ключи, пополнения и внутриигровые товары требуют category-by-category legal/payment/provider review.

## Категории цифрового доступа

Аккаунты, подписки, ключи, пополнения и внутриигровые товары нельзя оценивать одним общим правилом. Для каждой исходной платформы и способа поставки нужен отдельный результат review.

Текущий MVP-порядок:

- категория по умолчанию имеет статус `restricted` и недоступна для публикации;
- одного заявления продавца «аккаунт мой» недостаточно: условия исходного сервиса должны прямо допускать передачу или перепродажу;
- украденные, взломанные, восстановленные у третьего лица, совместно используемые или полученные обманом аккаунты запрещены;
- запрещены аккаунты с чужими персональными данными, активными платёжными средствами, чужой перепиской или иными данными, которые нельзя безопасно удалить до передачи;
- запрещены обход региональных ограничений, злоупотребление trial/promo, использование поддельной личности и передача вопреки ToS исходного сервиса;
- передача логина и пароля для услуги пополнения не считается безопасным способом исполнения;
- официальный gift code, ключ или пополнение через разрешённый канал может быть допущен только при подтверждённом источнике, праве распространения и одобрении платёжного провайдера;
- до review FanFuel не обещает покупателю, что сторонний сервис сохранит переданный аккаунт, подписку или цифровое право.

Перед включением конкретной подкатегории нужны `LEGAL_REVIEW_REQUIRED`, `PAYMENT_REVIEW_REQUIRED`, правила модерации, доказательства происхождения, refund/dispute policy и явное решение в `docs/DECISIONS.md`.
...
- Допускает ли провайдер оплату аккаунтов, подписок, ключей, пополнений и внутриигровых товаров по каждой подкатегории?
...
## UX-TASK-041 — поддержка автора из выручки продавца

Quote возвращает fingerprint условий и суммы. Новый checkout передаёт его при создании заказа: изменение цены/условий/выбранной поддержки после просмотра вызывает 409, а не молчаливый перерасчёт покупки. Для старых API-клиентов поле пока необязательно. Advisory transaction lock сериализует повторные запросы с одинаковым buyer+idempotency key до вызова провайдера.

Решение пользователя от 22.09.2026: цена покупателю не увеличивается. Продавец задаёт `promo_bps` и `storefront_bps`, где 0 < promo < storefront <= 5000. Старое поле `affiliate_percent_bps` остаётся совместимым, но не является разрешением начислять деньги. До явной настройки старые товары имеют `commission_configured=false`; перед публикацией требуется настройка. Все опубликованные разрешённые товары участвуют в промокодах, витрина — только выбранные товары.

Один заказ поддерживает одного автора. При разных получателях витрины и промокода сервер требует `attribution_choice=storefront|promo|none`. При одном авторе действует более высокая ставка витрины. Доступность профиля, включённость канала, членство товара в витрине и отсутствие самореферала проверяет сервер. Получатель, канал, ставка и сумма сохраняются в snapshot заказа. Округление вниз в minor units; остаток остаётся продавцу. Расчёт производится с итоговой цены после скидок. Никаких float для денег.

Это mock-контур. Реальные сборы провайдера, налоги и тариф платформы остаются отдельной политикой PaymentProvider/settlement, LEGAL_REVIEW_REQUIRED. Новая миграция не пересчитывает прошлые заказы и ledger. Денежные проводки используют зафиксированное распределение, возврат отменяет исходные проводки. Promo — механизм поддержки, не обещание скидки.

### Граница текущей реализации UX-TASK-041

Allocation записывается в snapshot/order и mock deal. Это не реализованный production ledger или выплата. RefundShare подготовлен как чистая функция; production refund adapter ещё не подключён. В текущем mock API сбор платформы равен нулю; настоящий тариф/налоги не определяются этим значением. Нового скидочного механизма нет. Старые commission_configured=false предложения не участвуют в поддержке до настройки. Формулировка «весь каталог» относится к настроенным разрешённым товарам, а не к обходу этих проверок.
