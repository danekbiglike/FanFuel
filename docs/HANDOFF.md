...
## Наблюдение за мостом VM, 24.09.2026, FF-0010

По запросу владельца добавлен `infra/scripts/monitor-vm-bridge.ps1` и установлен в `C:\ProgramData\FanFuel\monitor-vm-bridge.ps1`. Задание Windows `FanFuel - Monitor VM Bridge` запускается от `remot` в режиме S4U каждые две минуты. Через SSH оно проверяет доступность edge VM и FanFuel VM, затем из каждой VM проверяет связь с роутером; для FanFuel принудительно используется мостовой интерфейс `enp0s8`. Первую неудачу помечает `degraded`, вторую подряд — `alert`, успешную проверку — `healthy`. Состояние атомарно записывается в `C:\ProgramData\FanFuel\vm-bridge-state.json`; локальные адреса и пути к ключам хранятся вне репозитория в `C:\ProgramData\FanFuel\vm-bridge-monitor.json`. Скрипт не перезапускает VM и не меняет сеть.

Автоматизация Codex `Контроль сети FanFuel` (ID `fanfuel`) раз в десять минут читает этот файл и уведомляет о подтверждённом сбое, устаревшем состоянии или восстановлении; при неизменной норме остаётся тихой. Она не является независимым мониторингом, если хост или среда Codex недоступны. Первое ручное и первое плановое выполнение завершились с кодом `0`: `edgeSsh`, `edgeRouter`, `appSsh`, `appRouter` равны `true`. Ближайший плановый запуск после проверки был назначен на 19:18 МСК. Проверить состояние: `Get-Content C:\ProgramData\FanFuel\vm-bridge-state.json`; задание: `Get-ScheduledTaskInfo -TaskName 'FanFuel - Monitor VM Bridge'`. Следующий шаг при повторении: сверить событие NDIS/Intel I211 и результат обеих VM до ручного переподключения моста. Автоматические тесты проекта не запускались; выполнена эксплуатационная проверка установленного задания.

## Уточнение сетевого инцидента: мост VirtualBox, 24.09.2026, FF-0010 / UX-TASK-044

Итог повторной попытки в 15:54 МСК: по разрешению владельца переподключена привязка `oracle_VBoxNetLwf` к физическому Ethernet. Сценарий выключил мостовой компонент на три секунды и включил его снова; финальное состояние `Enabled=True`. Предыдущие запуски не меняли сеть: Windows PowerShell 5.1 не смог разобрать файл UTF-8 без BOM с русским текстом. После исправления кодировки сценарий завершился с кодом `0`.

Связь восстановлена без изменения правил роутера, перезапуска VM и действий ПАКТ: edge `192.168.0.250` и FanFuel VM `192.168.0.108` получили по два ответа от роутера `192.168.0.1`; edge установил исходящее TCP-соединение с `1.1.1.1:443`. Внешние проверки TCP 80 и 443 успешны по 5/5 узлов: `https://check-host.net/check-report/4d343a37k3c4` и `https://check-host.net/check-report/4d343a43k7fe`. `https://playground.fanfuel.ru/` отвечает HTTP 200 из 5/5 внешних точек (`https://check-host.net/check-report/4d343d29k531`), `https://fanfuel.ru/` — HTTP 200 из 3/3 (`https://check-host.net/check-report/4d344592k229`). Восстановление после перепривязки подтверждает локальный отказ пути VirtualBox bridge как непосредственную причину нынешней недоступности. Что именно вызвало первоначальное зависание моста — сброс Intel I211, фильтр VirtualBox или их взаимодействие — пока не доказано. Заявка ПАКТ №678149 может оставаться открытой; её статус отдельно не проверялся. Проверка на iPhone после восстановления не проводилась. Автоматические тесты проекта не запускались: код не менялся.

Проверка по замечанию владельца изменила оценку причины. `mail.danechka.com`, `fanfuel.ru` и `playground.fanfuel.ru` указывают на один публичный IP `194.37.1.112`. С независимых внешних узлов TCP 587 и 993 почтового сервера доступны, а TCP 80 и 443 обоих сайтов дают тайм-аут. Следовательно, действующий Static NAT для этого IP не отказал целиком; заявка ПАКТ №678149 остаётся открытой, но считать провайдера единственной причиной веб-сбоя нельзя. Проверки: `https://check-host.net/check-report/4d335184k55e`, `https://check-host.net/check-report/4d33517ck7f9`, `https://check-host.net/check-report/4d337d14kd6a`, `https://check-host.net/check-report/4d337d1ak189`.

В Archer C5400X правила TCP 80 и 443 включены и направлены на edge `192.168.0.250`; журнал показывает их успешное создание после плановой загрузки в 04:00. Контроль доступа и привязка IP/MAC выключены; ARP-таблица роутера содержит edge. Диагностика роутера получила четыре ответа от почтового устройства `192.168.0.141`, но ни одного от edge `192.168.0.250` и FanFuel VM `192.168.0.108`. Захват пакетов на edge увидел ICMP и TCP SYN от Windows-хоста `192.168.0.104`, но не увидел ICMP от роутера и внешние SYN на 80/443. Обе VM доступны хосту и друг другу, однако не могут установить соединение через мост с роутером или интернетом; FanFuel VM выходит наружу только через отдельный адаптер VirtualBox NAT. Это указывает на неисправность локального мостового пути между физической сетью и VirtualBox. Точную точку потери кадров между роутером, Intel I211 и фильтром VirtualBox пока не локализовали.

В Windows в 04:00:30 есть событие NDIS 10400: драйвер Intel I211 сообщил, что оборудование перестало отвечать, и сбросил сетевой интерфейс; линк восстановился в 04:00:37. Совпадение по времени и одинаковый отказ двух VM делают это сильной гипотезой причины сбоя моста, но не доказательством дефекта конкретного драйвера. Краткое переподключение виртуального кабеля только edge VM проблему не устранило; после него LAN HTTPS снова ответил 200. Настройки роутера не менялись. Если сбой повторится, сначала проверить достижимость роутера из обеих VM и события Intel I211/NDIS, затем отдельно исследовать драйвер и фильтр VirtualBox.

## Локальный адрес playground для телефона по Wi-Fi, 24.09.2026

Пока ПАКТ восстанавливает входящую трансляцию по заявке №678149, на edge добавлен отдельный LAN-only HTTP-вход на порту 8085 к статическому playground. Он слушает только LAN-адрес edge, допускает домашнюю подсеть и проксирует FanFuel VM по HTTPS с проверкой сертификата. Роутер пробрасывает только 80/443; новый порт не опубликован в интернете. Основные Host/SNI маршруты edge и FanFuel VM не менялись. Источники: `infra/nginx/fanfuel-playground-lan.conf.template`, `infra/scripts/enable-playground-lan.sh`; описание: `docs/PLAYGROUND.md`, `docs/DEPLOYMENT.md`, уточнение ADR-047 в `docs/DECISIONS.md`.

На edge сохранена копия `/etc/nginx/nginx.conf.before-playground-lan-20260924073520`. `sh -n` скрипта и `nginx -t` прошли. Через LAN новый вход отдаёт HTML 200, JS/CSS/Worker 200; прежние `/health` на 8080 и публичный vhost через прямой LAN edge продолжают отвечать 200. Страница открыта в браузере, ошибок в консоли нет, персонаж отрисовывается и обновляет состояние. Прямая проверка на iPhone по Wi-Fi не прошла: edge VM не получил соединение от телефона и не смог пинговать его, тогда как Windows-хост видит телефон. Для обхода добавлен `infra/scripts/playground-lan-proxy.mjs`, слушающий только LAN IP Windows-хоста и передающий GET/HEAD на edge. На 24.09 процесс запущен на `192.168.0.104:8085` (PID 30256); локальные HTML/JS/Worker отвечают 200, владелец сообщил, что адрес открылся на iPhone. После перезагрузки Windows процесс потребуется запустить снова. HTTP-вход предназначен только для статической демоверсии без учётных записей и покупок. Для отката edge удалить установленный server block и его точный include, проверить `nginx -t`, перезагрузить nginx либо восстановить указанную резервную копию. Связь: `UX-TASK-044`, `FF-0010`.

## ПАКТ подтвердил сбой Static NAT на своём пограничном шлюзе, 24.09.2026

Технический специалист ПАКТ ответил в 07:05 МСК по обращению **№678149**: услуга Static NAT активна, закреплённый публичный IP не изменится; ПАКТ зафиксировал сбой трансляции на своём пограничном шлюзе и передал заявку сетевым инженерам для восстановления маппинга адресов. Это подтверждает, что частный WAN-адрес домашнего роутера сам по себе штатен, а локальный проброс портов менять не требуется. Специалист попросил приложить отчёт внешней проверки через форму «Обратная связь».

Повторная проверка в 07:07 МСК из Санкт-Петербурга, Стамбула и Сан-Паулу всё ещё дала `Connection timed out` для playground: `https://check-host.net/check-report/4d2287f4k234`. Текстовый отчёт для ПАКТ подготовлен вне репозитория и в 07:09 МСК прикреплён в чат с техническим специалистом с просьбой включить его в заявку №678149. Специалист подтвердил, что отправка файла через чат подходит; подтверждение его добавления сетевым инженерам и последующая проверка восстановления ещё открыты. Связь: `UX-TASK-044`, `FF-0010`.

## Публичный доступ всё ещё недоступен: входящий маршрут ПАКТ, 24.09.2026

Повторная проверка после восстановления LAN показала: `playground.fanfuel.ru` и `fanfuel.ru` отвечают HTTPS 200 при прямом обращении к edge внутри сети, но недоступны через публичный DNS-адрес. Из нескольких внешних проверочных точек HTTP и TCP 80/443 завершаются тайм-аутом; тот же результат у другого сайта, который использует этот роутер. Это исправляет прежний слишком широкий вывод о «восстановленном доступе»: восстановлена только локальная цепочка VM → edge, публичная доступность не подтверждена и сейчас отсутствует.

В TP-Link Archer C5400X правила TCP 80 и 443 на edge включены. WAN роутера имеет частный адрес из `10.0.0.0/8`, тогда как внешние сайты показывают публичный адрес провайдера. Владелец подтвердил, что услуга внешнего IP (Static NAT) уже подключена и частный WAN-адрес на роутере отображался и раньше. Следовательно, частный WAN сам по себе не является причиной сбоя; нужно проверить привязку действующей услуги и входящий маршрут на стороне ПАКТ. Сайт и nginx не выключались. У роутера включена еженедельная перезагрузка по четвергам в 04:00, и журнал этого дня начинается после загрузки в 04:00. Совпадение по времени не доказывает, что перезагрузка вызвала сбой Static NAT. Утрата DHCP-аренды VM в 05:20 — отдельный локальный сбой, уже устранённый статическим адресом.

Проверено: DNS имён, внешний HTTP/TCP, HTTPS через локальный edge, правила Virtual Servers, WAN-адрес и журнал роутера. Изменений в настройках роутера и провайдера не внесено. Для восстановления публичного доступа ПАКТ должен проверить, куда направляет входящий трафик действующей услуги Static NAT и не изменилась ли её привязка к WAN-сеансу; локальная перенастройка роутера по текущим данным не обоснована. После исправления повторить независимую проверку 80/443, обоих HTTPS-доменов и страницы с iPhone по Wi-Fi и мобильной сети. Связь: `UX-TASK-044`, `FF-0010`.

Владелец подтвердил, что сейчас playground не открывается на iPhone ни по Wi-Fi, ни по мобильной сети. С его разрешения в 06:49–06:50 МСК отправлены подробное обращение через официальную форму `pakt.ru/contacts/` и сообщение в онлайн-чат технического отдела. Форма показала подтверждение принятия и обещание ответить по указанной владельцем почте. Технический специалист Евгений В. в чате подтвердил получение обращения, сначала назвал номер 678150, затем исправил его: **правильный номер 678149**. Ему предложено проверить привязку Static NAT к текущему WAN-сеансу. Технический ответ и восстановление маршрута на момент записи ещё не получены. Контактные данные владельца в Git не записывались.

В 07:00 МСК повторная независимая HTTPS-проверка из Германии, Сербии и США снова завершилась `Connection timed out`; отчёт `https://check-host.net/check-report/4d223b4ck4c0` передан специалисту в чат. До положительной внешней проверки инцидент остаётся открытым. Для продолжения запросить у ПАКТ результат проверки по №678149 и затем повторить проверку сайта; не считать получение номера заявки восстановлением доступа.

## Восстановлен маршрут к playground и основному сайту, 24.09.2026

Причина простоя: `enp0s8` VM FanFuel потерял DHCP-аренду 24.09 в 05:20:42 МСК и остался без IPv4. `tcpdump` на VM увидел DHCP Discover с запросом прежнего `192.168.0.108`, но не увидел Offer; штатное обновление аренды не помогло. Статический адрес на этом же интерфейсе сразу восстановил связь с edge, значит nginx, playground и обычный LAN-обмен исправны. Журналы не показывают ручного выключения; они не позволяют доказать, потерялся ли ответ на роутере или при передаче DHCP через мост VirtualBox.

На VM закреплён адрес `192.168.0.108/24` и маршрут через `192.168.0.1` в netplan; `enp0s3` с NAT DHCP оставлен для SSH. Перед заменой конфигурации сделана копия `/etc/netplan/50-cloud-init.yaml.before-20260924-dhcp-loss`, `netplan generate/apply` прошли. Cloud-init больше не управляет сетевым файлом при запуске. Конфигурация обновлена только на VM, исходники приложения и контейнеры не изменялись. Обновлены `docs/DEPLOYMENT.md`, игнорируемый `docs/SERVER_ACCESS.md`, `docs/CHANGELOG.md` и этот handoff. Связь: `UX-TASK-044`, `FF-0010`.

Проверки: через edge `playground.fanfuel.ru` и `fanfuel.ru` отвечают HTTPS 200; актуальные HTML, JS, CSS и Worker playground также отвечают 200. nginx active, все девять контейнеров запущены, шесть с healthcheck healthy. `networkctl` показывает статический IPv4 и маршрут. Прямая проверка по внешнему IP с этого Windows-хоста упирается в отсутствие NAT loopback; независимая проверка из внешней сети и перезагрузка VM не выполнялись. Следующее: при доступе к журналам роутера проверить DHCP-отказы около 05:20 МСК, затем отдельно проверить после плановой перезагрузки VM. Архитектурное исправление не зависит от результата этой проверки.

## Недоступность публичного playground, 24.09.2026

По вопросу владельца проверены обе VM и публичный маршрут. Playground на FanFuel VM не выключен: nginx active, release `/var/www/fanfuel-playground/current` на месте, локальный HTTPS отвечает 200. Edge nginx работает, но не может подключиться к upstream `192.168.0.108:443`; то же затрагивает основной `fanfuel.ru`. Журнал `systemd-networkd` фиксирует `enp0s8: DHCP lease lost` 24.09.2026 в 02:20:42 UTC (05:20:42 МСК). У интерфейса нет IPv4, журнал sudo за 02:00–02:25 UTC не содержит ручных действий. Штатный `networkctl renew enp0s8` выполнен, но адрес не получен; доступность публичного сайта не восстановлена. Непосредственная причина потери аренды требует проверки DHCP/роутера или VirtualBox bridge. Принудительный IP и перезапуск VM не применялись.

## UX-TASK-043 — согласование движения глаз и силуэта, 24.09.2026

По отзыву владельца глаз заметно сильнее менял ось между рисунками, чем корпус. Причина: дополнительная плавная вариация применялась только к округлым замкнутым путям. В `packages/pencil-engine/src/contour.ts` она добавлена для всех замкнутых контуров; коэффициент округлой формы уменьшен до `0,75`, силуэта установлен на `0,6` ширины кисти с учётом `handwork` и длины. Глаза и корпус остаются независимыми рисунками, частота перерисовки не меняется.

Нижний стык округлой формы проверен на десяти кадрах и на обычном размере Ойли. Полное наложение двух концов создавало тёмный узел. Суммарный вес проходов теперь ограничен; короткий внутренний конец рисуется одним узким волокном, а не тремя полными. Место стыка продолжает меняться от рисунка к рисунку. Изменены `src/contour.ts`, `src/contour.test.ts`, сравнительные изображения в `packages/pencil-engine/reference`, `docs/PENCIL_ENGINE.md`, `docs/PENCIL_ENGINE_VISUAL_REWORK.md`, `docs/tasks/ui-ux/UX-TASK-043.md`, `docs/DECISIONS.md`, `docs/CHANGELOG.md` и этот handoff. TASK-ID: `UX-TASK-043`.

Проверки: 18/18 адресных тестов контура, typecheck, сборки demo/playground и ESLint прошли. Общий набор пакета: 39/40; прежний неуспешный тест `soft-body.test.ts` про разницу растяжения при разных настройках мягкости не относится к изменённому рендеру контура. Публичный playground не обновлялся. Остаётся оценить видимость стыка и баланс движения при обычном размере на экране владельца; локальный увеличенный лист не заменяет художественное принятие. Замеры кадрового бюджета на слабом устройстве также открыты.

## UX-TASK-043 — непрерывная передача округлого стыка, 24.09.2026

По отзыву владельца стык округлого контура был неравномерным: иногда утолщался, иногда выглядел как просвет. В `packages/pencil-engine/src/contour.ts` исправлен порядок передачи между двумя концами: до встречи первый остаётся на исходной оси, после встречи её принимает второй; только вспомогательный короткий хвост отклоняется внутрь, становится тоньше и светлее. Ослаблено накопление графита в общей зоне. На каждом новом `redrawFrame` стык и плавное отклонение петли по-прежнему меняются. В `src/contour.test.ts` добавлен тест непрерывной передачи. Обновлены сравнительные листы, `PENCIL_ENGINE.md`, `PENCIL_ENGINE_VISUAL_REWORK.md`, `UX-TASK-043`, ADR-046 и `CHANGELOG.md`. TASK-ID: `UX-TASK-043`.

Проверки: 18 адресных тестов контура прошли. Общий набор пакета сейчас содержит один неуспешный тест мягкого тела о разнице растяжения при двух значениях мягкости; он относится к параллельным изменениям `soft-body.ts` и этим изменением контура не затрагивался. Typecheck, сборки demo/playground, адресный ESLint и `git diff --check` прошли. Художественное принятие стыка остаётся открытым; следующий просмотр должен оценить несколько последовательных кадров при обычном масштабе.

## UX-TASK-043 — учёт закреплённого узла в ограничениях связей, 24.09.2026

После адаптивного разбиения владелец сообщил, что дефект исчез на одной грани и проявился на другой. Найдена общая причина в solver: коррекция пружины делилась поровну между двумя концами, хотя узел хвата затем жёстко возвращался к курсору. Это обходило предел растяжения и передавало деформацию соседним рёбрам. Теперь при закреплённом конце всю коррекцию получает свободный конец. Обновлены `PENCIL_ENGINE.md`, `PENCIL_ENGINE_RUNTIME.md`, changelog и этот handoff. TASK-ID: `UX-TASK-043`.

Проверки: `npm run typecheck -w @fanfuel/pencil-engine`, сборки demo/playground, адресный ESLint и `git diff --check` прошли. Тесты не запускались. Визуальный сценарий с разными длинными гранями нужно повторно оценить в браузере.

## UX-TASK-043 — перерисовка глаза на каждом кадре, 24.09.2026

Владелец уточнил, что предыдущая корректировка ошибочно замедлила перерисовку. В `packages/pencil-engine/src/contour.ts` выбор плавного изгиба и короткого стыка округлой петли вновь выполняется на каждом `redrawFrame`. Уменьшена только амплитуда поперечного отклонения до `0,9 × widthCss × handwork` с коэффициентом длины; короткая третья волна отсутствует. Нажим сохраняется по штриху, общий `redrawStrengthCss` для округлых форм ослаблен до 35 %. Тест проверяет изменение при новом `redrawFrame` и сохранение узнаваемого овала на 24 кадрах. Обновлены сравнительный лист, `PENCIL_ENGINE.md`, `PENCIL_ENGINE_VISUAL_REWORK.md`, `UX-TASK-043`, ADR-046 и `CHANGELOG.md`. TASK-ID: `UX-TASK-043`.

Проверки: 39 тестов пакета, typecheck, сборки demo/playground, адресный ESLint и визуальное сравнение выполнены. Художественный результат ещё не принят владельцем; оценивать в живом `contour.html`, особенно короткий нижний стык и размер покадрового изменения.

## UX-TASK-043 — адаптивное подразделение физической границы, 24.09.2026

По предложению владельца длинные края следует делить на меньшие физические участки, сохраняя общее растяжение. В `packages/pencil-engine/src/soft-body.ts` добавлен вариант `createSoftBody(bounds, options)`: `maxSegmentLength` адаптирует размер решётки в пределах 5–9 узлов на ось, `maxSpringStretch` ограничивает растяжение каждой связи отдельно. Числовая форма `createSoftBody(bounds, columns, rows)` сохраняет прежнюю сигнатуру и явное разрешение. Демо запускает сетку с шагом 28 CSS px и локальным пределом 1,45. Это проба общего подхода, а не финальная настройка. Описания добавлены в `PENCIL_ENGINE.md`, `PENCIL_ENGINE_RUNTIME.md`, ADR-046, `UX-TASK-043`, changelog и этот handoff. TASK-ID: `UX-TASK-043`.

Проверки: `npm run typecheck -w @fanfuel/pencil-engine`, сборки demo/playground, адресный ESLint и `git diff --check` прошли; тесты не запускались. Нужен просмотр длинного края при перетаскивании и измерение стоимости повышенной сетки на мобильном устройстве.

## UX-TASK-043 — умеренная неровность глаза и внутренний стык, 24.09.2026

Владелец отметил, что предыдущая коррекция сделала глаза слишком ровными. В `packages/pencil-engine/src/contour.ts` возвращена умеренная плавная асимметрия, но её основная часть закреплена за `id`/seed штриха, а малая добавка обновляется по `boilFrame`, обычно дважды в секунду. Быстрый покадровый сдвиг округлой формы остаётся ослабленным. Стык теперь рисуется двумя коротко перекрывающимися концами: один остаётся на исходной дуге, второй тоньше и светлее, отклоняется внутрь и меняется с первым местами без внешнего выступа или белой дырки. Обновлён `reference/rounded-join-study.png`, связанная документация и ADR-046. TASK-ID: `UX-TASK-043`.

Проверки: 39 тестов пакета прошли, включая контроль 24 кадров овального глаза; typecheck, сборки demo/playground, адресный ESLint и визуальное сравнение листа и двух кадров сцены прошли. Художественное совпадение с фото владельца ещё не подтверждено. Следующее — оценить стык и глаз в обычном масштабе, затем при необходимости дать художнику возможность явно указывать место соединения в ассете.

## UX-TASK-043 — стабильная форма округлого глаза, 24.09.2026

По сравнению с серией ручных овалов была проверена проба увеличить процедурную асимметрию всей петли. Владелец сразу отметил, что глаза стали волнистыми и быстро менялись, поэтому проба отменена. В `packages/pencil-engine/src/contour.ts` оставлено только малое отклонение оси, постоянное по `id`/seed; покадровый сдвиг округлой формы составляет 35 % общей настройки. Плавная составляющая нажима тоже фиксируется по штриху. Нижний стык переделан как короткое наложение двух концов с небольшим внутренним пересечением, без отдельной линии и внешнего узла. В `src/contour.test.ts` добавлена проверка 24 кадров на сохранение овальной формы. Обновлён `reference/rounded-join-study.png` и описание в `PENCIL_ENGINE.md`, `PENCIL_ENGINE_VISUAL_REWORK.md`, `UX-TASK-043`, `DECISIONS.md`, `CHANGELOG.md`. Исходные пути, коллайдер и заливка не менялись. TASK-ID: `UX-TASK-043`.

Проверки: 39 тестов пакета, typecheck, сборки demo/playground, адресный ESLint и визуальный просмотр двух кадров живой сцены прошли. Старый и новый сравнительные листы сопоставлены с фото владельца: идеальные входные эллипсы всё ещё визуально правильнее ручного рисунка; это ожидаемое ограничение такого входного пути. На слабом устройстве производительность не замерялась. Следующее — художественная оценка короткого стыка на глазах и ввод авторских путей/мест соединения при подготовке ассета; не возвращать крупную покадровую деформацию всей петли.

## UX-TASK-043 — отменён общий предел растяжения, 24.09.2026

Владелец уточнил, что общее вытягивание за курсор — ожидаемая и интересная реакция «рогатки». Удалён недавно добавленный предел растяжения и перенос тела после достижения предела; прежний локальный мягкий хват с offset и весами соседей сохранён. Дефект относится к длинной грани, чья деформация повторяет штрих и выглядит как тонкая нить. Возможные дальнейшие подходы: (1) сохранить длину материала вдоль длинных прямых сегментов, сжимая поперечную ось; (2) дать длинной грани отдельный вес деформации/сопротивление на уровне соответствующих пружин; (3) разделить штрих по сегментам и включать повтор только на коротких участках, а длинный отрезок переносить как один штрих. По просьбе владельца исправление не вносилось. Обновлены документация движка/runtime, changelog и этот handoff. TASK-ID: `UX-TASK-043`.

Проверки: `npm run typecheck -w @fanfuel/pencil-engine`, сборка demo, адресный ESLint и `git diff --check` прошли. Тесты не запускались. Следующий шаг — согласовать подход к длинной грани отдельно; не вводить глобальную длину растяжения.

## UX-TASK-043 — предел растяжения за курсором, 24.09.2026

На снимке владелец показал, что при длинном перетаскивании за кончик Ойли превращался в нить через всю сцену. Причина — локальный узел следовал за курсором без ограничения накопленного растяжения. В `packages/pencil-engine/src/soft-body.ts` добавлен предел длины растяжения, зависящий от габарита и мягкости тела. После предела избыток движения переносит решётку целиком, сохраняя длину силуэта. Обновлены `PENCIL_ENGINE.md`, `PENCIL_ENGINE_RUNTIME.md`, `CHANGELOG.md` и этот handoff. TASK-ID: `UX-TASK-043`.

Проверки: `npm run typecheck -w @fanfuel/pencil-engine`, сборка demo, адресный ESLint и `git diff --check` прошли. Тесты не добавлялись и не запускались. Следующее — проверить захват кончика и длинное перетаскивание в браузере; важно убедиться, что тело следует за курсором после ограничения и не теряет мягкость.

## UX-TASK-043 — округлый стык без «прыщика», 24.09.2026

По новой фотографии владельца с серией овалов в `packages/pencil-engine/src/contour.ts` удалён процедурный угол на нижней дуге. Он выпячивал силуэт, тогда как в образцах два окончания обычно расходятся и накладываются **вдоль** плавного контура. Исходная линия в короткой зоне по-прежнему заменена двумя концами без непрерывной подложки. Их штрихи слегка различаются тоном, сужаются на концах, частично накапливают графит, а второй проходит преимущественно внутри петли с крошечным переходом через ось. Знак внутренней нормали берётся из площади контура. Амплитуда отклонения варьирует по кадрам от почти нулевой до умеренной; 96 % выборов стыка остаются на нижних 5 % петли. Сравнительный лист из десяти овалов сохранён в `packages/pencil-engine/reference/rounded-join-study.png`, четыре кадра живого Ойли просмотрены рядом с фото. Никаких исходных путей, заливки или физических коллайдеров не меняли. Обновлены `src/contour.ts`, `src/contour.test.ts`, `reference/rounded-join-study.png`, `PENCIL_ENGINE.md`, `PENCIL_ENGINE_RUNTIME.md`, `PENCIL_ENGINE_VISUAL_REWORK.md`, `UX-TASK-043`, `DECISIONS.md`, `CHANGELOG.md` и этот handoff. TASK-ID: `UX-TASK-043`.

Проверки: тесты пакета, typecheck, сборки demo/playground, адресный ESLint и визуальная проверка. Художественное принятие владельцем остаётся открытым. Отдельная будущая задача: дать художнику возможность указать предпочтительную область стыка в ассете.

## UX-TASK-043 — локальный мягкий захват на кончиках, 24.09.2026

Владелец заметил чрезмерное вытяжение при перетаскивании за самый кончик. `grabSoftBody` теперь сохраняет смещение курсора относительно выбранного узла, поэтому хват не телепортирует узел на указатель. На время хвата заранее рассчитываются веса соседних узлов; `moveSoftBodyGrab` распределяет смещение локально и ослабляет перенос остальной решётки. Это должно убрать острый вытянутый «клюв», сохранив мягкую деформацию. Документация алгоритма обновлена в `PENCIL_ENGINE.md`, `PENCIL_ENGINE_RUNTIME.md`, `CHANGELOG.md` и этом handoff. TASK-ID: `UX-TASK-043`.

Проверки: `npm run typecheck -w @fanfuel/pencil-engine`, сборка demo, адресный ESLint и `git diff --check` прошли; тесты не добавлялись и не запускались. В браузере нужно сравнить захват самой вершины и центра тела, а также убедиться, что pointer down не вызывает скачка.

## UX-TASK-043 — корректировка горизонтального зажима, 24.09.2026

Владелец сообщил, что при зажиме между карточками демо всё ещё показывало сильный удар о край. Причина: первая версия измеряла только уменьшение решётки, но её пружины удерживают ширину почти прежней. В `packages/pencil-engine/src/soft-body.ts` оценка сжатия теперь учитывает свободный зазор между парой контактных граней вместе с фактическим размером тела. Встречные проекции разных поверхностей сами подтверждают зажим; сильная скорость удара сохраняет сильную классификацию. Обновлены описание алгоритма в `PENCIL_ENGINE.md`, `PENCIL_ENGINE_RUNTIME.md`, ADR-046 и `UX-TASK-043`, changelog и этот handoff. TASK-ID: `UX-TASK-043`.

Проверки: `npm run typecheck -w @fanfuel/pencil-engine`, сборки demo/playground, `npm run lint -- --no-cache` и `git diff --check` прошли. Тесты не добавлялись и не запускались. Интерактивное воспроизведение горизонтального зажима остаётся необходимым.

## UX-TASK-043 — два конца самой округлой линии, 24.09.2026

Владелец уточнил, что нижний стык не должен быть отдельной линией рядом с готовой обводкой. В `packages/pencil-engine/src/contour.ts` для компактной замкнутой петли исходная линия локально делится на два затухающих конца. Они слегка перекрываются и пересекаются; сплошная линия под ними в зоне стыка не рисуется. После прямого сравнения с приложенной фотографией первая версия разбиения оказалась корявой: тёмный узел и излом появлялись на нижнем боку глаз. После следующего отзыва зону выбора сузили до нижних 2,5 % формы, боковое отклонение уменьшили, плотность и толщину распределили между концами и добавили неглубокий локальный угол только на достаточно больших петлях. Серия из четырёх перерисовок помогла убрать крючок на уже изогнутом правом глазу; зрачки почти не получают угол. Выбор стороны и смена положения по нарисованным кадрам сохранены. Механизм стыков угловатых и открытых форм не менялся; отдельный повторный проход остаётся независимым эффектом. Изменены `src/contour.ts`, `src/contour.test.ts`, `PENCIL_ENGINE.md`, `PENCIL_ENGINE_RUNTIME.md`, `PENCIL_ENGINE_VISUAL_REWORK.md`, `UX-TASK-043`, `DECISIONS.md`, `CHANGELOG.md` и этот handoff. TASK-ID: `UX-TASK-043`.

Проверки: тесты пакета 38/38, typecheck, сборки demo/playground, ESLint и `git diff --check`. Визуально проверены увеличенные области глаз в статическом и интерактивном образцах. Художественная мера при обычном масштабе и скорость на слабом устройстве требуют оценки владельца; публичный playground не обновлялся.

## UX-TASK-043 — распознавание сдавливания между поверхностями, 24.09.2026

После замечания владельца выяснилось, что предыдущая правка удерживала приоритет сильного удара, но не распознавала само раздавливание. В `packages/pencil-engine/src/soft-body.ts` добавлен edge-trigger детектор встречных контактов разных поверхностей: он сравнивает текущий габарит решётки с исходным и возвращает `SoftBodyImpact.squeeze` при сжатии от 8 %; от 18 % реакция сильная. Пока контакт непрерывен, повторных событий нет. В `demo/soft-body-demo.ts` сжатие имеет высший приоритет внутри эпизода, показывает пару поверхностей и процент, а сильная деформация ориентируется по оси давления. Тексты добавлены в `demo/locales.ts`. Обновлены `PENCIL_ENGINE.md`, `PENCIL_ENGINE_RUNTIME.md`, `DECISIONS.md`, `UX-TASK-043`, `CHANGELOG.md` и этот handoff. TASK-ID: `UX-TASK-043`.

Проверки: `npm run typecheck -w @fanfuel/pencil-engine`, сборки demo/playground, `npm run lint -- --no-cache` и `git diff --check` прошли. Тесты не добавлялись и не запускались. Интерактивную физическую сцену ещё нужно посмотреть с зажимом между низом карточки и полом, боковыми гранями карточек, а также обычным касанием без заметного сжатия. Публичный playground не обновлялся.

## UX-TASK-043 — стык виден в живом и неподвижном контуре, 24.09.2026

Владелец не увидел нижний стык круглых объектов. Причины: второй штрих лежал внутри ширины основного и большой сравнительный лист не использовал `handwork`. В `packages/pencil-engine/src/contour.ts` усилены короткий дополнительный отпечаток и поперечное смещение округлой петли; 92 % выборов ограничены нижними 20 % её высоты. Основной контур не прерывается. Режим бумажного зерна теперь поддерживает `handwork`, а в `demo/contour-scene.ts` он включён только на глазах и зрачках неподвижного Ойли. На увеличенных локальных кадрах снизу видны небольшие двойные участки; на обычном масштабе требуется повторный просмотр владельца. Обновлены `src/contour.test.ts`, `PENCIL_ENGINE.md`, `PENCIL_ENGINE_RUNTIME.md`, `PENCIL_ENGINE_VISUAL_REWORK.md`, `UX-TASK-043`, `DECISIONS.md`, `CHANGELOG.md` и этот handoff. TASK-ID: `UX-TASK-043`.

Проверки: 38/38 тестов пакета, typecheck, сборки demo/playground, ESLint и `git diff --check` прошли. Большой неподвижный лист строился примерно за 195 мс в отдельном headless Chrome на этом ПК; это не показатель скорости живого малого кадра или слабого устройства. Публичный playground не обновлялся.

## UX-TASK-043 — арбитраж последовательных столкновений, 24.09.2026

По отзыву владельца сильный удар мог сменяться «мягким приземлением» после отскока или касания соседней грани. В `packages/pencil-engine/demo/soft-body-demo.ts` столкновения сгруппированы в эпизоды: каждое новое касание продлевает окно 750 мс, а ранг реакции внутри него может только повышаться от мягкого края до сильной опоры сверху. Повторное слабое касание не перезаписывает статус сильного удара; перетаскивание или перемещение стрелками сбрасывает эпизод. Скоростной порог и физическая симуляция `stepSoftBody` не менялись. Обновлены документация движка/runtime, ADR-046, UX-TASK-043, changelog и этот handoff. TASK-ID: `UX-TASK-043`.

Проверки: typecheck, сборки demo/playground и ESLint запускались; тесты не запускались в этой итерации. Локальную последовательность удар→отскок→мягкая посадка проверить интерактивно после применения изменений. Публичный playground не обновлялся.

## UX-TASK-043 — аккуратный стык округлых форм, 24.09.2026

По отзыву владельца округлые части Ойли выглядят слишком ровно. В `packages/pencil-engine/src/contour.ts` компактная замкнутая петля определяется по отношению площади к периметру после деформации. При включённом `handwork` для неё выбирается один короткий стык, чаще на нижней дуге; дополнительный случайный повтор на такой петле не строится. Длина наложения ограничена долей периметра, основная линия не прерывается. В `demo/soft-body-demo.ts` стык глаз виднее, чем зрачков, без изменения исходных путей. Добавлен тест приоритета нижней дуги и сохранения прежнего выбора у прямоугольника. Обновлены `PENCIL_ENGINE.md`, `PENCIL_ENGINE_RUNTIME.md`, `PENCIL_ENGINE_VISUAL_REWORK.md`, `UX-TASK-043`, `DECISIONS.md`, `CHANGELOG.md` и этот handoff. TASK-ID: `UX-TASK-043`.

Проверки: 37/37 тестов пакета, typecheck, сборки demo/playground, ESLint и `git diff --check` прошли; локальный стенд просмотрен в обычном и увеличенном масштабе. После художественного просмотра владельцем проверить повёрнутые и сильно деформированные эллипсы, затем измерить кадр на слабом устройстве/OBS. Публичный playground не обновлялся.

## Живой графический ритм после удаления символов, 24.09.2026

После замечания владельца о незавершённом виде публичных экранов добавлены неровные карандашные подчёркивания и контуры: выбранная категория, заметка у Ойли, ряд направлений, презентационная лента, пустые состояния и тестовая площадка. У товарных заглушек теперь разные рисунки для дизайна, OBS, обучения и услуг, без выдуманной обложки и повторного крупного названия. Удалены оставшиеся видимые стрелки в заметках. Изменены `apps/web/src/app/editorial.css`, `apps/web/src/components/marketplace-experience.tsx`, `product-cover.tsx`, `role-presentation.tsx`, `packages/pencil-engine/playground/index.html`, `main.ts`, новый `ink.css`; обновлены `UI_RULES.md`, `DESIGN_SYSTEM.md`, `DESIGN_AUDIT.md`, `PAGE_SPECS.md`, `UX-TASK-040`, `CHANGELOG.md` и этот файл. TASK-ID: `UX-TASK-040`, `UX-TASK-044`.

Проверки: web typecheck, production build, ESLint изменённых компонентов, сборка playground и поиск видимых звездочек/стрелок прошли. Локально просмотрены desktop light и mobile dark marketplace с подставленными данными, мобильная презентация автора и playground. Реальный iPhone, живой каталоговый API и художественное принятие владельцем ещё не проверены. Следующее: review на устройстве и с реальными товарами, затем точечно скорректировать плотность рисунков при необходимости.

## UX-TASK-043 — стык без белого просвета, 24.09.2026

По последнему уточнению владельца `packages/pencil-engine/src/contour.ts` больше не вырезает дырку в контуре. Основные волокна непрерывны и лишь немного ослабевают в месте стыка; короткий второй жест проходит рядом со смещением и асимметричными концами. Места меняются между нарисованными кадрами и выбираются с учётом геометрии. Пиксельная маска просвета удалена. Повторы на длинных ровных линиях и выходы за острый угол сохранены. В `demo/locales.ts` ползунок переименован в «Стыки и повторы» / «Joins and retraces», старт остаётся 50 %. Обновлены тесты непрерывности/вариативности и документация. Изменены `src/contour.ts`, `src/contour.test.ts`, `demo/locales.ts`, `PENCIL_ENGINE.md`, `PENCIL_ENGINE_RUNTIME.md`, `PENCIL_ENGINE_VISUAL_REWORK.md`, `UX-TASK-043`, `DECISIONS.md`, `CHANGELOG.md` и этот handoff. TASK-ID: `UX-TASK-043`.

Проверки: тесты пакета 36/36, typecheck, сборки demo/playground, ESLint и `git diff --check` прошли. В локальном браузере просмотрено начальное значение 50 % и сравнение с отключённым эффектом. Измерение слабого устройства/OBS и окончательное художественное принятие ещё нужны; публичный playground не публиковался.

## UX-TASK-043 — достоверные стыки карандашного жеста, 24.09.2026

Промежуточная проба по предыдущему уточнению владельца: в `packages/pencil-engine/src/contour.ts` прежнее светлое пятно заменили двумя асимметричными концами штриха с узким настоящим просветом. В следующей итерации владелец отклонил этот просвет, и текущая реализация описана выше. Анализ замкнутого шва добавляет верхушку Ойли к кандидатурам острого угла; один или два тонких жеста иногда проходят за вершину на 1–3 CSS px. Вероятность и длина меняются по кадрам, исходная ось не меняется. Интенсивность и частота повторного прохода достигают прежнего максимума при 50 % `handwork`; ползунок `demo/contour.html` теперь стартует с 50 %. Заливка, физика, коллайдер и production-страницы не менялись.

Ориентиры: сравнение с карандашным рисунком владельца и наблюдения за перекрывающимися/соединяемыми отметками в [Grimm, Sketch study](https://web.engr.oregonstate.edu/~grimmc/content/SketchStudy/SketchStudy.pdf) и ограничениями простого разбиения по кривизне в [Lewis и др.](https://scribblethink.org/Work/Graphite05/BETTERSTROKES.pdf). Эти работы не задают готовых чисел для нашей кисти; параметры подобраны художественно.

Проверки: 36/36 тестов пакета, typecheck, сборки demo/playground, полный ESLint и `git diff --check` прошли. В локальном браузере проверены начальные 50 % и возвращение Ойли в обычную позу. Контрольный headless Chrome на этом ПК для 220 CSS px/DPR 2 дал медиану около 6,1 мс с эффектом и 6,2 мс без него за 40 прогретых кадров; это отдельный тест, не замер всей сцены и не оценка слабого устройства. Публичный playground не публиковался. Следующее — художественный просмотр владельцем на нескольких размерах/кадрах, затем замер на слабом устройстве/OBS и кэширование неизменных частей. TASK-ID: `UX-TASK-043`.

## UX-TASK-043 — разрывы и повторные проходы по геометрии жеста, 24.09.2026

В `packages/pencil-engine/src/contour.ts` добавлен параметр кисти `handwork`: краткий отрыв грифеля выбирается преимущественно у поворотов, концов и перемен нажима, а слабый повторный проход — в середине длинного ровного участка. Наличие и место заново определяются для каждого нарисованного кадра воспроизводимым seed; при движении метки следуют за материалом штриха. В `demo/soft-body-demo.ts`, `demo/contour.html` и `demo/locales.ts` добавлен локализованный ползунок «Разрывы и повторы» (0–100 %, старт 65 %); глаза затронуты слабее, улыбка без эффекта. Исходники `playground/main.ts` получают отдельный номер рисунка при движении. Исходные пути, заливка, коллайдер и физика не менялись. Обновлены `PENCIL_ENGINE.md`, `PENCIL_ENGINE_RUNTIME.md`, `PENCIL_ENGINE_VISUAL_REWORK.md`, `DECISIONS.md`, `CHANGELOG.md` и этот файл.

Проверки: 34/34 теста пакета, включая выбор разных мест на 24 кадрах и разную вероятность у поворота/длинной прямой; typecheck пакета, demo и playground; обе production-сборки; полный ESLint; `git diff --check` — успешно. В открытом локальном `contour.html` проверены умеренные 65 % и максимальные 100 %, затем возвращены 65 %. Малый кадр Worker показывал около 3,9–4,0 мс на текущем ПК; холодная статическая сборка — около 122 мс. Это не замер слабого устройства. Публичный playground не обновлялся. Следующее — художественная оценка владельца при нескольких размерах и в движении, затем профиль на слабом устройстве/OBS и кэш неизменных частей. TASK-ID: `UX-TASK-043`.

## UX-TASK-043 — читаемые глаза и заметное сглаживание, 24.09.2026

По последнему отзыву владельца в интерактивном Ойли усилили тон тонких деталей лица: в `demo/soft-body-demo.ts` и `playground/main.ts` для глаз, зрачков и улыбки снижена зависимость от слабого нажима, увеличена плотность контакта; у улыбки сокращено сужение. В `src/geometry.ts` сглаживание заменено равномерной дискретизацией и локальным усреднением всей деформированной оси с ограниченным сдвигом. `src/contour.ts` допускает `smoothingCss` до 3 CSS px. В `demo/contour.html`, `demo/locales.ts` и контроллер добавлен локализованный ползунок «Сглаживание контура» (0–100 %, старт 70 % = 2,1 CSS px). Исходные пути, физический коллайдер и заливка не менялись. Обновлены документы `PENCIL_ENGINE*`, `DECISIONS.md`, задача `UX-TASK-043` и changelog.

Проверки: 31/31 тестов пакета, включая смягчение небольшой ряби, typecheck, сборки demo/playground, ESLint и `git diff --check` прошли. В открытом локальном стенде проверены видимость правого глаза и улыбки, а также заметное различие при 0 и 100 % сглаживания; затем возвращены стартовые 70 %. Малый кадр Worker занимал около 3,6–4 мс на этом ПК. Не проверены художественное принятие владельцем в движении, слабое устройство/OBS и публичная площадка; публикации не было. Следующее — оценка владельцем на разных размерах и профилирование слабого устройства. TASK-ID: `UX-TASK-043`.

## UX-TASK-043 — более тёмный нажим и плавные углы, 24.09.2026

По отзыву владельца прежний контур оставался серым и почти одинаковым при слабом и сильном нажиме. В `packages/pencil-engine/src/contour.ts` нормировка потока графита теперь использует базовый размер отпечатка: рост нажима увеличивает одновременно ширину и плотность, а порог контакта с бумагой реагирует заметнее. В `demo/soft-body-demo.ts` размах авторского профиля мягкой кисти увеличен до 1,55. `src/geometry.ts` строит короткую квадратичную дугу у излома вместо переноса одной вершины; локальный стенд и исходники playground используют `smoothingCss: 1.6`. Исходные пути, физика, коллайдер, заливка и расписание перерисовки не менялись. Добавлен тест на разницу тона ядра при слабом/сильном нажиме, обновлены `PENCIL_ENGINE.md`, `PENCIL_ENGINE_RUNTIME.md`, `PENCIL_ENGINE_VISUAL_REWORK.md`, `DECISIONS.md`, `UX-TASK-043` и changelog.

Проверки: 30/30 тестов пакета и typecheck прошли; сборки demo/playground, ESLint и `git diff --check` проведены после изменений. В открытом локальном `contour.html` визуально проверены сильнее различающийся тон и скругление; малый кадр Worker показывал около 3,8 мс на текущем ПК. Не проверены художественное принятие владельцем, слабый телефон, OBS и публичная площадка; исходники площадки обновлены, публикации не было. Следующее — принять или точнее откалибровать переход нажима по образцу пера, затем проверить разные масштабы и слабое устройство. TASK-ID: `UX-TASK-043`.

## UX-TASK-043 — материал графита и слабое сглаживание, 24.09.2026

Сделано по отзыву владельца: в `packages/pencil-engine/src/contour.ts` контакт движущегося штриха получил вытянутую вдоль жеста среднюю/широкую фактуру, нормировку расхода графита по шагу отпечатков и небольшое влияние нажима на контакт с бумагой. В `src/geometry.ts` добавлено ограниченное сглаживание ломаной: открытые концы и стык замкнутого контура сохраняются. Демо и исходники playground передают `smoothingCss: 1` через Worker и синхронный fallback. Исходные пути, коллайдер, заливка и частота двух обновлений в секунду в покое не менялись. Обновлены `PENCIL_ENGINE.md`, `PENCIL_ENGINE_RUNTIME.md`, `PENCIL_ENGINE_VISUAL_REWORK.md`, `DECISIONS.md`, задача `UX-TASK-043` и changelog.

Проверки: 29/29 тестов пакета, включая ограничение сдвига оси, typecheck пакета/демо/playground, сборки demo и playground, полный ESLint и `git diff --check` прошли. Локальное демо проверено в браузере на `http://127.0.0.1:5174/contour.html`; малый подвижный кадр Worker показывал около 3,6–4 мс на этом ПК, а холодная статическая сборка менялась от 105 до 222 мс между перезагрузками. Это не замер слабого устройства и не гарантия художественного сходства. Production playground не публиковался. Следующее: сравнить вид при 180/512/1000 CSS px с авторскими штрихами, отдельно проверить тонкие детали и движение, измерить на слабом устройстве/OBS, затем кэшировать неизменный материал. TASK-ID: `UX-TASK-043`.

## Применение запрета звездочек и стрелок — 24.09.2026

По прямому запросу владельца убраны видимые звездочки и стрелки из web UI, ru/en словарей и тестовой площадки Ойли. Заменены декоративные символы и стрелки в CSS, подписях ссылок, кнопках, карточках и игре. Для перестановки блоков редактора теперь видны существующие локализованные подписи; игровые плитки с прежней звездной формой показывают кольцо и квадрат. Также удалены звездный элемент favicon и декоративный луч в иллюстрации. Технические обработчики клавиш сохранены.

Изменены web-компоненты, затронутые страницы и стили, словари common.json, словарь playground, файлы playground, DESIGN_AUDIT.md, CHANGELOG.md и этот handoff. Проверки: typecheck и production-сборка web, адресный ESLint и production-сборка playground прошли; повторный поиск видимых Unicode-символов и CSS escape совпадений не дал. Реальный iPhone и опубликованный сайт не проверялись. Следующее — проверить внешний вид на iPhone перед публикацией. TASK-ID: нет, прямой запрос владельца.

## Карандашный знак канистры — 24.09.2026

Сделано: знак FanFuel перерисован в `apps/web/src/components/brand-art.tsx`: лаймовая канистра с F получила живой графитный контур и сдержанные карандашные штрихи. Сохранены исходный `viewBox`, размер компонента и цветовые токены, поэтому знак подхватывается существующими местами использования.

Файлы: `apps/web/src/components/brand-art.tsx`, `docs/DESIGN_ASSETS.md`, `docs/CHANGELOG.md`, `docs/HANDOFF.md`.

Проверки: автоматические проверки не запускались. Ограничения: вид при малом размере и художественное принятие владельцем ещё не проверены в браузере. Следующее: оценить знак в шапке и в крупных карточках бренда.

## Правила текста и символов в интерфейсе — 24.09.2026

Зафиксирован запрет на звездочки и стрелки в пользовательском интерфейсе и тексте: Unicode-символы, эмодзи и декоративные пиктограммы этих форм могут некорректно отображаться на iPhone и воспринимаются как шаблонное оформление. Добавлено требование писать естественно, конкретно и без декоративных элементов и оборотов, похожих на автоматически сгенерированный текст. Обновлены `AGENTS.md`, `docs/UI_RULES.md` и `docs/UX_WRITING.md`; пользовательское содержимое и код не менялись.

Проверки не запускались: изменена только документация. Следующее — применять правило при создании и редактировании интерфейсов и текстов.

## UX-TASK-043 — заметное зерно дважды в секунду, 24.09.2026

По уточнению владельца в покое нужны именно два обновления в секунду, а предыдущая смена только мелких точек выглядела почти неподвижной. В `packages/pencil-engine/demo/soft-body-demo.ts` интервал возвращён с 2000 на 500 мс, `boilFrame` меняется с тем же периодом. В `src/contour.ts` по этому номеру пересоздаются мелкий и средний вытянутый уровни графитного контакта; широкий уровень остаётся стабильным. Стартовый ползунок «Живость штриха» 60 % теперь даёт `boilStrength: 0.75`. Лёгкое отклонение оси на каждом показанном кадре движения не изменялось. Обновлены пояснения `ru/en`, `PENCIL_ENGINE*`, ADR-046, задача `UX-TASK-043` и changelog. Исходная геометрия, физика, заливка и production-сайт не менялись.

Проверки: пакетные тесты 28/28, включая проверку, что при фиксированной оси между кадрами меняется заметная доля пикселей и материал переезжает с линией; typecheck, сборка демо, полный ESLint и `git diff --check` прошли. В браузере новый материал открыт на `http://127.0.0.1:5174/contour.html`; пробный малый кадр Worker занимал около 2,6–3,1 мс на текущем ПК. Художественное принятие владельцем и замер на слабом устройстве/OBS остаются открытыми. Дальше — проверить вид в покое и движении вместе с владельцем и затем кэшировать неизменные части. TASK-ID: `UX-TASK-043`.

## UX-TASK-043 — живая перерисовка карандашного контура, 24.09.2026

По отзыву владельца перерисовка после возврата графитной фактуры оставалась слишком слабой. В `packages/pencil-engine/src/contour.ts` добавлены `redrawFrame` и `redrawStrengthCss`: новый показанный рисунок получает небольшое плавное поперечное отклонение оси, а зерно продолжает следовать за штрихом. В `demo/soft-body-demo.ts` и Worker в покое рисунок обновляется раз в две секунды, при движении — на каждом показанном кадре до 24 раз/с. Ползунок «Живость штриха» (старт 60 %, 0 % отключает эффект) управляет отклонением и долей нового мелкого зерна; подписи добавлены в `ru/en`. Исходные пути, физический коллайдер, заливка и production-сайт не менялись. Обновлены инструкции `PENCIL_ENGINE*`, ADR-046, задача `UX-TASK-043` и changelog. ESLint теперь игнорирует `dist-playground/` как уже игнорируемый Git сгенерированный каталог.

Проверки: 28/28 тестов пакета (в том числе детерминизм нового кадра и ограниченный размах), typecheck пакета и playground, сборка демо, полный ESLint и браузерный стенд прошли. По атрибутам canvas проверено, что в покое меняются `redrawFrame` и `boilFrame`; новый кадр малого контура в Worker занимал около 2,6–2,7 мс на этом ПК. Не проверены художественное принятие владельцем, реальный планшет, слабое устройство и OBS. Следующее — оценка видимой силы перерисовки при перетаскивании и в покое, затем измерение на слабом устройстве и кэширование частей. TASK-ID: `UX-TASK-043`.

## UX-TASK-044 — публичная тестовая главная с Ойли, 24.09.2026

Сделано: `https://playground.fanfuel.ru/` опубликован на существующих edge nginx и FanFuel VM. Это статическая сборка Vite из `packages/pencil-engine/playground`, без нового процесса, контейнера или публикации незавершённого Next.js сайта. Есть четыре явно фиктивные карточки, отключённые вход/поиск/категории, интерактивный карандашный Ойли и панель настроек штриха/мягкости. Язык ru/en, semantic light/dark; цены форматирует общий i18n helper. При тёмной теме пигмент контура становится светлым; reduced-motion показывает статический кадр. Верхняя граница удерживает Ойли в viewport после резкого броска.

Файлы: `packages/pencil-engine/playground/*`, `packages/pencil-engine/vite.playground.config.ts`, `packages/pencil-engine/tsconfig.playground.json`, `packages/pencil-engine/package.json`, `packages/pencil-engine/demo/soft-body-{demo,worker}.ts`, `packages/pencil-engine/src/soft-body{,.test}.ts`, `packages/i18n/src/playground.ts`; `infra/nginx/fanfuel-playground-{http,tls}.conf`, `infra/scripts/enable-playground-edge.sh`; `docs/PLAYGROUND.md`, `docs/tasks/ui-ux/UX-TASK-044.md`, Page Map/Specs, tracker, User Flows, UX Implementation Status, Architecture, Deployment, Decisions ADR-047, Tasks, README, CHANGELOG и `.gitignore` для каталога сборки. Не связанные ранее существовавшие изменения рабочей копии сохранены.

Проверки: 27/27 тестов движка; typecheck движка и i18n; production Vite build; ESLint затронутых TS файлов; `git diff --check`. В Playwright на 390/1440 px проверены отсутствие переполнения и ошибок, перетаскивание, ползунки, ru/en, обе темы и reduced-motion. Публичный HTTPS отдаёт 200 с noindex/CSP, браузер загружает JS/Worker без ошибок; основной `https://fanfuel.ru/` отвечает 200. `nginx -t` успешен на обеих VM. Сертификат `playground.fanfuel.ru` выпущен до 22.12.2026, `certbot.timer` активен, симуляция продления с `--no-random-sleep-on-renew` прошла. На Windows настроены DNS `111.88.96.50/51`, которые в эту проверку не ответили; DNS роутера `192.168.0.1` и `8.8.8.8` вернули правильный A-record. Поэтому прямая навигация из Chrome на этой машине может завершиться тайм-аутом, хотя публичный сервер исправен. Для независимой проверки маршрута использован `curl --resolve`, а публичный ACME HTTP-01 прошёл штатно. Локальную DNS-конфигурацию Windows эта задача не меняла.

Что не проверено: художественное принятие контура владельцем, слабые мобильные устройства и длительная работа физики. Следующий шаг: визуальный review площадки, профилирование на слабом телефоне, затем отдельная интеграция с настоящими карточками основной главной. Текущая страница не является реальным каталогом и не открывает payment/API-сценарии. TASK-ID: `UX-TASK-044`.

## UX-TASK-043 — вернуть вид карандашного штриха в движении, 24.09.2026

По отзыву владельца сглаженное зерно в интерактивном образце выглядело серым маркером. В `packages/pencil-engine/src/contour.ts` контакт в `grainSpace: "stroke"` заменён на многоуровневое разрывистое зерно в координатах самого штриха: мелкая доля меняется по `boilFrame`, средняя и широкая остаются прикреплёнными к материалу. Порог контакта и накопление графита теперь такие же, как в режиме бумажного зерна. В `demo/soft-body-demo.ts` стартовая кисть согласована с полем «Нарисовать пером»: плотность 0,85, зерно 0,45, без бокового двоения и плотной радиальной маски. Обновлены подписи `ru/en`, документация рендера и задача `UX-TASK-043`. Геометрия контура, мягкая физика и заливка не менялись.

Проверки: 26/26 тестов пакета, typecheck, сборка демо и полный ESLint прошли. В браузере `http://127.0.0.1:5174/contour.html` проверены неподвижный контур, перетаскивание и отсутствие новых ошибок; малый кадр Worker занимал примерно 2,4–2,7 мс на этом ПК. Тест переноса зерна вместе с линией и смены дискретного кадра проходит. Не проверены слабое устройство/OBS и художественное принятие владельцем. Следующий шаг — оценить живой штрих на стенде вместе с владельцем; после принятия перейти к сохранённым частям, кэшу и интеграции actor runtime. TASK-ID: `UX-TASK-043`.

## UX-TASK-043 / UX-TASK-038 — фактура следует за штрихом, настраиваемая мягкость, 23.09.2026

Сделано: в `packages/pencil-engine/src/contour.ts` добавлен режим `grainSpace: "stroke"` с ориентированными вдоль карандашной оси волокнами и необязательной исходной длиной `materialLengthCss`. Он используется только в интерактивном стенде: фактура движется вместе с деформированным штрихом, а её мелкая часть обновляется два раза в секунду независимо от частоты показанной геометрии (до 24 кадров/с). Прежний режим бумажного зерна оставлен для статичного контурного листа. В `src/soft-body.ts` убрано принудительное выпрямление в воздухе; корпус сохраняет угол до посадки. Добавлен `setSoftBodySoftness(body, 0..1)` и ползунок мягкости на стенде, стартовое значение 0,6. Обновлены ru/en подписи, `PENCIL_ENGINE.md`, `PENCIL_ENGINE_RUNTIME.md`, `PENCIL_ENGINE_WEB_STAGE.md`, `DECISIONS.md`, `tasks/ui-ux/UX-TASK-043.md` и `CHANGELOG.md`.

Проверки: `npm test -w @fanfuel/pencil-engine` (26 тестов, включая перенос текстуры с линией, сохранение угла в полёте, влияние мягкости и опору самого мягкого режима в ограниченной сцене); `npm run typecheck -w @fanfuel/pencil-engine`, `npm run build:demo -w @fanfuel/pencil-engine`, `npm run lint -- --no-cache`, `git diff --check`. В браузере на `http://127.0.0.1:5174/contour.html` проверены новый вид контура, перетаскивание и изменение ползунка. Небольшой кадр в Worker на этом ПК занимал около 3–4 мс. Не проверено: художественное принятие владельцем, слабое устройство и OBS. Следующее: оценить результат в движении вместе с владельцем, затем кэшировать неизменную массу штриха и отдельно оптимизировать движущуюся фактуру. Заливка и production `OilyStage` не менялись.

## UX-TASK-043 / UX-TASK-038 — редкая смена зерна в покое и контакт по контуру, 23.09.2026

Сделано: стенд `packages/pencil-engine/demo/contour.html` теперь строит новое мелкое зерно два раза в секунду, когда Ойли стоит, и не чаще 24 раз/с при движении. Физика остаётся на фиксированном шаге 1/120 с. `src/contour.ts` меняет бумажный контакт по `boilFrame`, сохраняя общую форму, профиль нажима и базовую фактуру; при повторе номера кадра результат детерминирован. Доступны новый мягкий графит, прежняя кисть и наконечник, импортированный через верхний контрол. `src/soft-body.ts` привязывает контакт к деформированному замкнутому силуэту, проверяет верх/бока/низ DOM-карточек, удерживает знак площади ячеек, мягко восстанавливает форму после удара и не усыпляет тело под заметным наклоном. При перетаскивании часть смещения передаётся всему корпусу, чтобы быстрое движение курсора действительно переносило персонажа; остаток даёт упругое растяжение. В `demo/soft-body-demo.ts`, `demo/soft-body-worker.ts`, `demo/locales.ts` добавлены раздельный темп рисунка, локализованный статус бокового контакта и параметры кистей. Обновлены `PENCIL_ENGINE.md`, `PENCIL_ENGINE_RUNTIME.md`, `PENCIL_ENGINE_WEB_STAGE.md`, `DECISIONS.md`, `tasks/ui-ux/UX-TASK-043.md`, `CHANGELOG.md`.

Проверки: `npm run typecheck -w @fanfuel/pencil-engine`, `npm test -w @fanfuel/pencil-engine` (22 теста), `npm run build:demo -w @fanfuel/pencil-engine`, `npm run lint -- --no-cache`, `git diff --check`; в локальном браузерном стенде проверены внешний вид, перетаскивание и контакт с карточкой. После успокоения счётчик показал 4–5 новых кадров за 2,2 секунды. На этом ПК видимый карандашный кадр небольшой области занимал около 2,5–3 мс в Worker; это не полный бюджет кадра и не замер слабого устройства. Локальный стенд оставлен на `http://127.0.0.1:5174/contour.html`: порт 5173 занят другим dev-сервером. Следующий шаг: кэшировать неизменную массу штриха и перестраивать только мелкое зерно, измерить производительность на слабом устройстве/OBS, затем соединить записанные пером части и жёсткую канистру с actor runtime. Production `OilyStage` не менялся; контур стенда остаётся упрощённой художественной пробой, заливки нет.

## UX-TASK-043 / UX-TASK-038 — свободное перетаскивание и мягкий контур, 23.09.2026

Сделано: в `packages/pencil-engine/src/soft-body.ts` добавлена решётка 5×5 с фиксированным шагом, гравитацией, drag constraint, верхними контактными гранями и событиями силы удара. `bindSoftBodyPath`/`deformSoftBodyPath` меняют геометрию самих осей карандашных штрихов; исходные кисть и профиль нажима сохраняются. В `demo/contour.html` добавлен отдельный стенд с двумя DOM-карточками, захватом за форму Ойли, управлением стрелками, коротким клипом сильного удара, показом силы удара и времени расчёта. `demo/soft-body-worker.ts` растеризует только область персонажа и хранит не более одного ожидающего кадра; при остановке формы новые кадры не запрашиваются. Новые тексты добавлены в `demo/locales.ts` для `ru/en`, цвета используют semantic tokens.

Проверки: `npm run typecheck -w @fanfuel/pencil-engine`, `npm test -w @fanfuel/pencil-engine` (19 тестов), `npm run build:demo -w @fanfuel/pencil-engine`; в headless Chrome проверены захват и отпускание Ойли на DOM-карточку. На этом ПК показан ориентир ~0,1 мс физики и ~1–2 мс построения небольшого карандашного кадра в Worker; это не метрика слабого устройства и не гарантия 24 fps. Заливка и production-код сайта не менялись. Изменены `src/soft-body.ts`, `src/index.ts`, тест, `demo/{contour.html,contour.ts,soft-body-demo.ts,soft-body-worker.ts,locales.ts,study.css}` и проектные документы `PENCIL_ENGINE*`, `DECISIONS`, `PAGE_SPECS`, `UI_UX_TRACKER`, `USER_FLOWS`, `UX_IMPLEMENTATION_STATUS`, `TASKS`, задачи 038/043 и `CHANGELOG`.

Ограничения: тестовая форма Ойли упрощена и не утверждена художественно; нынешний контурный рендер всё ещё перестраивает деформированную часть каждый показанный кадр; падение использует верхние плоскости карточек, а не полноценную 2D-коллизию. Массив исходного нажима сохраняется, но его расположение по длине деформированного пути может сдвигаться: нужна материальная привязка к исходным точкам. Жёсткая канистра/части лица, связанные с ригом, авторский полный клип приземления, произвольные карточки реального сайта, непрерывность фактуры при деформации, слабые устройства/OBS и переключение маршрутов требуют следующего среза. На стенде reduced-motion показывает неподвижную позу; эквивалент для сайта нужен при интеграции. Следующий шаг: записанные пером части и мягкий корпус внутри actor runtime, отдельная жёсткая канистра; затем `SurfaceRegistry` всех разрешённых карточек `/` и `/marketplace` с QA ввода/скролла и производительности. TASK-ID: `UX-TASK-043`, `UX-TASK-038`.

## UX-TASK-040 — авторское начертание FanFuel, 23.09.2026

По уточнению владельца `FF` остаётся сокращённым написанием названия, а не логотипом. На основе его срезанных верхних углов и плотных штрихов нарисованы отдельные векторные формы букв `FanFuel`; подготовлены светлый и тёмный варианты. Файлы `fanfuel-custom-lettering.svg`, `fanfuel-custom-lettering-preview.svg` и `fanfuel-custom-lettering-preview.png` лежат вне репозитория в каталоге визуализаций текущей задачи. Production-код не менялся. Проверка: SVG открыт в браузере, PNG просмотрен; `git diff --check` для handoff выполнен. Ограничение: это начертание одного названия, не шрифт с полным набором символов; требуется художественная оценка владельца перед применением в продукте. Следующий шаг — после принятия закрепить размеры, отступы и версии для тем в дизайн-системе. Связанная задача: `UX-TASK-040`.

## UX-TASK-040 — сокращённое написание FF, 23.09.2026

По запросу владельца подготовлен отдельный векторный вариант сокращённого написания `FF` в графитовом и лаймовом цветах утверждённого визуального направления. Файлы `ff-logo-concept.svg`, `ff-logo-preview.svg` и `ff-logo-preview.png` сохранены вне репозитория в каталоге визуализаций текущей задачи; production-код и действующий знак канистры с одной `F` не менялись. Проверка: SVG открыт в headless Chrome, PNG просмотрен на светлом и тёмном фоне. Позднее владелец уточнил статус `FF`: это сокращённое название, не логотип. Следующий шаг — сохранять это различие при оформлении брендовых материалов. Связанная задача: `UX-TASK-040`.

## UX-TASK-043 / UX-TASK-038 — DOM-сцена Ойли поверх сайта, 23.09.2026

Владелец уточнил цель actor runtime: Ойли должен воспринимать элементы существующего сайта как визуальные поверхности и разыгрывать сцены появления/падения и столкновения с карточкой товара. Проверен `apps/web/src/components/oily-stage.tsx`: сейчас один fixed actor использует `[data-oily-anchor]`, `ResizeObserver`, scroll/resize и CSS-переход, но не имеет сценариев, контактных поверхностей или нового карандашного рендера. Разделены сцены внутри `/`/`/marketplace` и будущая отдельная World-страница.

Добавлен `docs/PENCIL_ENGINE_WEB_STAGE.md` с `SiteStage`, `AnchorRegistry`, `SceneDirector`, контактными прокси, viewport/document координатами, событиями `intro-fall` и `bump → recover`, правилами ввода, reduced motion, производительности и следующим техническим срезом. Обновлены `docs/PENCIL_ENGINE_RUNTIME.md`, `docs/PENCIL_ENGINE.md`, ADR-045 в `docs/DECISIONS.md`, `docs/tasks/ui-ux/UX-TASK-038.md`, `UX-TASK-043.md`, `docs/TASKS.md`, `docs/UI_UX_TRACKER.md`, `docs/PAGE_SPECS.md`, `docs/USER_FLOWS.md`, `docs/UX_IMPLEMENTATION_STATUS.md` и `docs/CHANGELOG.md`. Production-код не менялся: текущий actor остаётся CSS/SVG до вертикального прототипа с записанным пером. Проверки: ссылки новых документов и `git diff --check` успешны; typecheck и 16 тестов пакета карандашного движка проходят. Сцены не проверялись в браузере, поскольку ещё не реализованы. Риски: невидимый «низ страницы» на длинном документе, sticky header, layout shifts, перехват кликов и холодная стоимость материала; для первой сцены выбран явный floor в пределах первого viewport. Дальше — реализовать один `intro-fall` с якорями, проверить четыре размера и reduced motion, затем одну карточку. TASK-ID: `UX-TASK-038`, `UX-TASK-043`.

## UX-TASK-043 — архитектура анимационного runtime, 23.09.2026

Владелец положительно оценил след в поле «Нарисовать пером» и предложил переиспользование спрайтов и анимаций по принципу 2D-игр для любых размеров экрана. Проверены текущие `packages/pencil-engine/src/model.ts`, `renderer.ts`, `contour.ts`, `player.ts`, `demo/scene.ts` и `demo/pen-pad.ts`: старый `PencilScene` содержит `group`, `sprite`, `motion`, но рендерит прежним методом; новый контурный след и JSON пера пока не являются частями этого дерева. В `docs/PENCIL_ENGINE_RUNTIME.md` описаны локальные штрихи/части, rig, сменные векторные рисунки, клипы, контроллер, размерные уровни кэша и режимы фактуры. Добавлен ADR-044 и обновлены `docs/PENCIL_ENGINE.md`, `docs/PENCIL_ENGINE_BRUSHES.md`, `docs/PENCIL_ENGINE_VISUAL_REWORK.md`, `docs/TASKS.md`, `docs/tasks/ui-ux/UX-TASK-043.md`, `docs/CHANGELOG.md`.

Production-код не менялся: это архитектурный этап. Проверены ссылки документа и согласованность контрактов с текущим кодом; пакетные проверки для существующего прототипа остаются следующей технической точкой после реализации вертикального среза. Риски: ширина `widthCss` при масштабировании готового bitmap, стоимость холодной подготовки, память кэша, качество смешивания поз и наклона пера. Первый прототип — две записанные пером части, `idle`/`wave` и смена глаза, один клип на 180/512/1000 CSS px с раздельным временем подготовки и кадра. Художественная оценка всего Ойли и тест слабого устройства/OBS остаются открытыми. TASK-ID: `UX-TASK-043`.

## UX-TASK-043 — импорт архивов кистей, настройки и запись пера, 23.09.2026

В `packages/pencil-engine/src/brush-import.ts` добавлен ограниченный импорт `.vbr` 1.0/1.5, sampled-масок `.abr` v6 и ZIP с `.abr`/`.vbr`/`.gbr`. Проверены два последних архива из «Загрузок» владельца: первый даёт 12 наконечников Photoshop, второй — 25 параметрических GIMP; файлы `.gdyn` не применяются. Большие маски ABR усредняются до 128 px по длинной стороне. Сторонние архивы остаются локальными, в репозиторий не скопированы. Зависимость `fflate` добавлена в пакет и lockfile.

В `src/contour.ts`, `demo/contour-scene.ts`, `demo/contour-worker.ts`, `demo/contour.ts`, `demo/contour.html`, `demo/study.css` и `demo/locales.ts` добавлены управление силой и размахом нажима, плавной детерминированной вариацией, длиной/минимумом сужения, seed и выбор импортированного наконечника. Геометрия Ойли и заливка не менялись. `demo/pen-pad.ts` добавляет поле рисования с `PointerEvent`/coalesced samples, сохранением и повторным открытием JSON версии 1. Исходные отсчёты нажима и наклона сохраняются; предпросмотр использует профиль нажима, но пока не использует наклон. Контактные размеры пера не считаются шириной кисти.

Проверки: `npm run typecheck -w @fanfuel/pencil-engine`, `npm test -w @fanfuel/pencil-engine` (16/16), `npm run build:demo -w @fanfuel/pencil-engine`, адресный ESLint — успешно. Headless Chrome: прямой импорт двух пользовательских ZIP (12+25 наконечников), выбор `.abr` и `.vbr`, смена параметров, экспорт/импорт JSON, синтетические события пера с тремя разными значениями нажима/наклона и запасной путь без Worker — без ошибок. Реальный графический планшет, точность наклона конкретного драйвера, художественное принятие разных наконечников и слабые устройства не проверялись. В одном прогоне холодная сборка 512 px на этом ПК заняла 132 мс, следующие импорты/настройки показывали примерно 8–92 мс; эти нестабильные числа не являются покадровым бюджетом. Следующее: испытать реальное перо, добавить использование наклона в модели материала и оценить кисти с владельцем перед анимационным этапом.

Инструкции автору и контракт данных: `docs/PENCIL_ENGINE_BRUSHES.md`; параметры API: `docs/PENCIL_ENGINE.md`. Связанная задача: `UX-TASK-043`.

## UX-TASK-043 — усиление нажима и концевой формы, 23.09.2026

По отзыву владельца предыдущий плавный профиль слишком слабо менял ширину и тон, а концы не сужались. В `packages/pencil-engine/src/contour.ts` диапазон `pressureProfile` расширен до 0,55–1,4; добавлен `PencilContourStroke.taper` со значениями `both/start/end/none`. Для открытых концов работает плавное сужение на участке около трёх базовых ширин кисти. В `demo/contour-scene.ts` усилены профили основных проходов Ойли и отключено сужение на внутренних стыках тела; пути и заливка не перерисовывались. В `src/contour.test.ts` добавлен тест видимого сужения концов и сохранения середины штриха. Обновлены снимки в `reference/`, документация API/метода и `UX-TASK-043`.

Проверки: `npm run test -w @fanfuel/pencil-engine` — 12/12; typecheck, сборка демо, адресный ESLint — успешно; браузерный рендер 180/512/1000 px без ошибок, запасной путь без Worker — успешно. В контрольном Chrome на 1000 CSS px/DPR 2 после холодного запуска полная сборка занимала 38–69 мс в четырёх повторениях; это не покадровый бюджет. Художественное принятие остаётся открытым. Дальше — оценка владельцем диапазона нажима и формы концов; полную регенерацию на каждый анимационный кадр по-прежнему не планировать.

## UX-TASK-043 — естественная изменчивость нажима, 23.09.2026

Владелец уточнил, что толщина и тон должны быть нестабильны, но как плавный жест, а не шум по коротким отрезкам. В `packages/pencil-engine/src/contour.ts` добавлен необязательный `PencilContourStroke.pressureProfile`: опорные значения 0,7–1,25 распределены по нормализованной длине пути и интерполируются плавной кубической кривой с ограничением выбросов. Без профиля действует слабая медленная детерминированная вариация. Профиль меняет радиус контакта и тон одновременно; ось и геометрия пути не изменяются. В `demo/contour-scene.ts` авторские профили заданы для основных проходов Ойли и канистры. Прежний зернистый трёхволоконный след сохранён, `fray: 0`.

В `src/contour.test.ts` добавлена проверка направленной вариации нажима на прямой; всего 11 тестов. Обновлены сравнительные снимки и документация API/визуального метода. Проверки: пакетные тесты 11/11, typecheck, сборка демо и адресный ESLint — успешно; браузерный рендер на 180/512/1000 px без ошибок, запасной путь без Worker — успешно. Прогретый полный расчёт 1000 CSS px/DPR 2 на этом ПК занял 32–50 мс в части повторений, были и более долгие выбросы; стабильного кадрового бюджета пока нет. Художественное принятие остаётся открытым. Следующий шаг — оценка владельцем характера нового профиля на стенде, без перерисовки контуров.

## UX-TASK-043 — возврат графитных волокон после отзыва, 23.09.2026

Владелец отверг промежуточный одноядерный образец как серый монотонный маркер и предпочёл фактуру варианта двумя итерациями раньше. В `packages/pencil-engine/src/contour.ts` восстановлены три близких графитных волокна и выраженный контакт с бумагой, слегка снижена итоговая непрозрачность. Стабильная ширина и нажим середины штриха сохранены; смещение оси, удалённые боковые волокна и дополнительные акценты не возвращены. `demo/contour-scene.ts`, остальные пути и заливка в этой итерации не изменялись. Обновлены `reference/contour-compare-{180,512,1000}.png` и крупный снимок `contour-render-1000.png`.

Проверки: `npm run test -w @fanfuel/pencil-engine` — 11/11; браузерный рендер трёх размеров без ошибок и запасной путь без Worker — успешно. Прогретая сборка в контрольном Chrome при 1000 CSS px/DPR 2: четыре замера 37,7/35,5/35,1/34,5 мс; это нагрузка полной статичной перестройки на одном ПК, не бюджет анимационного кадра. Статус художественного принятия открыт; дальше корректировать материал по оценке владельца, не меняя геометрию.

## UX-TASK-043 — стабилизация карандашного следа, 23.09.2026

Владелец принял направление геометрии и попросил её больше не перерисовывать; замечания касались чрезмерного контраста скана, двоения, ряби и случайных изменений толщины/яркости. В `packages/pencil-engine/src/contour.ts` убраны шумовой профиль нажима и смещение оси, концевое сужение уменьшено. Три основных отпечатка заменены одним контактным ядром; бумажное зерно действует мягче через прозрачность и кромку. В `demo/contour-scene.ts` выключены боковые волокна и отдельные акцентные проходы; укорочен только дублирующий конец левого прохода. Заливка, остальные геометрические пути и production UI не менялись. Обновлены сравнительные снимки 180/512/1000 px; предыдущий 512 px сохранён как `reference/contour-before-softening.png`.

Добавлен тест равномерности середины прямого штриха в `src/contour.test.ts`. Проверки: пакетные тесты 11/11, typecheck, сборка демо, адресный ESLint, браузерный рендер в трёх размерах и запасной путь без Web Worker. На контрольном Chrome при 1000 CSS px/DPR 2 после прогрева новая одноядерная кисть показывала ~32–34 мс в первых повторениях, с отдельными более долгими повторениями; замер нестабилен и не является гарантией 24 fps. Для 512 px/DPR 2 первые повторения ~12–14 мс. Физическое слабое устройство и OBS не проверялись. Дальше — получить оценку владельца по смягчённому образцу и уточнять только параметры материала; затем переносить принятый контур в анимационный формат.

## UX-TASK-043 — художественная доработка контура, 23.09.2026

По наложению на исходный рисунок исправлены контурные пути Ойли в `packages/pencil-engine/demo/contour-scene.ts`: видимый край тела, глаза, левая рука и нога. Граница тела построена из трёх частично перекрывающихся проходов и двух слабых акцентов. В `src/contour.ts` добавлены редкие боковые волокна `fray` и медленное смещение оси штриха; у тела выражены сильнее, чем у канистры. Старый стенд заливки и production UI не изменены. Обновлены `reference/contour-compare-{180,512,1000}.png`, добавлен крупный `contour-render-1000.png`; API и художественные ограничения описаны в `docs/PENCIL_ENGINE.md`, `docs/PENCIL_ENGINE_VISUAL_REWORK.md`, `docs/tasks/ui-ux/UX-TASK-043.md` и `reference/README.md`.

Проверки: `npm run typecheck -w @fanfuel/pencil-engine`, `npm run test -w @fanfuel/pencil-engine` (10/10), `npm run build:demo -w @fanfuel/pencil-engine`, адресный ESLint — успешно. В headless Chrome при DPR 2 прогретая фоновая сборка на этом ПК: примерно 5,5 мс для 180 px, 16 мс для 512 px и 36 мс для 1000 px; художественные проходы не ухудшили ранее измеренный бюджет 1000 px. Физическое слабое устройство и OBS в этой итерации не проверялись. Риск: точность путей остаётся ручной и приближённой; карандашный характер деталей на крупном масштабе ещё требует оценки владельца. Следующий шаг — уточнять характер грифеля и форму по конкретным замечаниям к трём размерам, затем готовить эталонный контур для анимационного формата.

## UX-TASK-043 — проверка и оптимизация нагрузки контура, 23.09.2026

По запросу владельца измерен новый отдельный рендер контура на 180/512/1000 CSS px при DPR 1/2, затем при эмуляции CPU ×4. Исходная прогретая сборка 1000 px/DPR 2 занимала около 66 мс, 512 px/DPR 2 — 22 мс. После ограничения рабочих пикселей областью путей, компактного хранения цвета и уменьшения числа отпечатков — около 36 и 16 мс соответственно на настольном headless Chrome. Для 1000 px/DPR 2 обработано около 2,0 млн пикселей вместо 4,0 млн; временные рабочие буферы сократились примерно с 84 до 19 МБ (без canvas/bitmap/накладных расходов браузера). Художественный вид проверен по снимкам, заливка не менялась.

В `demo/contour-worker.ts` добавлена фоновая сборка через OffscreenCanvas с переносом ImageBitmap. `demo/contour.ts` объединяет быстрые запросы ползунков, закрывает устаревшие bitmap и показывает время расчёта отдельно от задержки вывода; при отсутствии OffscreenCanvas использует прежний синхронный путь. Изменены `src/contour.ts`, `demo/{contour.ts,contour-worker.ts,locales.ts}`, docs и сравнительные снимки в `reference/`. Проверки: пакетные тесты, typecheck, production сборка Vite, браузерная проверка worker/fallback и быстрых изменений размера. Следующий шаг — добиваться художественной точности штриха, затем кэшировать неизменные контуры в анимации и проверить физическое слабое устройство/OBS. Эмуляция CPU ×4 дала около 63 мс для 512 px/DPR 2 и 175 мс для 1000 px/DPR 2 при полной перестройке: 24 fps так не достигаются, даже если UI остаётся отзывчивым благодаря worker. Статус `VISUAL_REWORK_REQUIRED` сохраняется.

Также добавлен частичный импорт `.gbr` v2 в новый контурный рендер: маска наконечника передаётся worker один раз и влияет на альфу следа. Проверены валидный и повреждённый GBR в браузере, 10 пакетных тестов. Просмотрен CC0-пакет из 60 GIMP/Krita кистей; карандашного наконечника для этого референса там не оказалось, архив не добавлен в проект. Встроенный процедурный след по-прежнему требует художественного сравнения; полная совместимость пресетов GIMP/Procreate не реализована.

## UX-TASK-043 — контур как отдельный первый этап, 23.09.2026

Владелец уточнил задачу: качество карандашной обводки первично, заливку пока не менять; ширина грифеля должна быть независимой от размера Ойли. Добавлен отдельный `packages/pencil-engine/src/contour.ts`: векторная ось штриха, ширина `widthCss` в CSS px, давление, три волокна, воспроизводимый контакт с бумагой и прозрачный результат. Новый `demo/contour.html` сравнивает только контуры с исходным референсом, даёт размеры 180/512/1000 px, ширину и зерно. Прежний рендер заливки и production UI не менялись. Снимки в `packages/pencil-engine/reference/contour-compare-*.png`.

Файлы: `src/{contour.ts,contour.test.ts,index.ts}`, `demo/{contour.html,contour.ts,contour-scene.ts,locales.ts,study.html,study.ts,study.css,study-scene.ts}`, `vite.config.ts`, `reference/README.md`, `docs/{PENCIL_ENGINE.md,PENCIL_ENGINE_BRUSHES.md,PENCIL_ENGINE_VISUAL_REWORK.md,DECISIONS.md,CHANGELOG.md,HANDOFF.md}`, `docs/tasks/ui-ux/UX-TASK-043.md`. Проверки: `npm run test -w @fanfuel/pencil-engine` — 9/9; `npm run typecheck -w @fanfuel/pencil-engine` — успешно; `npm run build:demo -w @fanfuel/pencil-engine` — успешно; адресный ESLint — успешно; Playwright/Chrome — три размера. Контрольная прямая с шириной кисти 4 CSS px дала 3–4 видимых px при размерах рисунка 180/512/1000 px (допуск растра), а 8 px дала более широкий след.

Ограничения: контур пока не принят художественно; геометрия Ойли приближённая, местами видны пересечения отдельных путей. Процедурные волокна всё ещё нужно сравнить с реальным графитным штрихом на крупном фрагменте; `.gbr` импорт из прежнего стенда не подключён к новому рендеру. Последний последовательный замер статичной генерации в основном потоке: 65 мс на первом размере 180 px, затем 14 мс при 512 px и 23 мс при 1000 px в headless Chrome без CPU throttling. Это не измерение слабого ПК и не обещание 24 fps. Следующий шаг: художественная настройка контуров и кисти по отзывам владельца, затем профиль памяти/ускорение и только после принятия контура — заливка.

## Handoff 2026-09-23: сухой контур и кадровый кэш образца (UX-TASK-043)

Обратная связь владельца: закраска почти подходит, но тело кажется вырезанным; обводка канистры устраивает, обводка Ойли была похожа на размытую водой. В `packages/pencil-engine/src/brush-study.ts` для тела добавлен отдельный сухой контур с детерминированной неровностью границы и мелкими отметками графита/бумаги; канистра продолжает использовать прежний контур. В `demo/study-scene.ts` добавлена лёгкая меняющаяся микрофактура. В `demo/study.ts`, `study.html` и `locales.ts` введены кэш двух подготовленных размеров/кистей, режим 24 кадра/с, счётчик пропусков и раздельные показатели сборки/вызовов рисования. Обновлены снимки `reference/study-compare-{180,512,1000}.png`, README, `docs/PENCIL_ENGINE.md`, `docs/PENCIL_ENGINE_BRUSHES.md`, `docs/PENCIL_ENGINE_VISUAL_REWORK.md`, `docs/tasks/ui-ux/UX-TASK-043.md`, `docs/DECISIONS.md`, `docs/CHANGELOG.md` и этот handoff.

Проверки: `npm run test -w @fanfuel/pencil-engine` — 8 PASS; `npm run typecheck -w @fanfuel/pencil-engine`, `npm run build:demo -w @fanfuel/pencil-engine`, точечный ESLint и `git diff --check` — PASS. Headless Chrome без page errors: кэш возвращает слой при повторном размере, анимация меняет пиксели и останавливается кнопкой. При DPR 2 и эмуляции CPU ×4 в коротком прогоне 512 и 1000 CSS px дали по 39 нарисованных кадров за ~1,6 с без пропусков; среднее время вызовов рисования 0,6–0,7 мс/кадр. Холодная сборка заняла 2,1 и 3,5 с соответственно. Это измерение JS/Canvas-вызовов и короткой браузерной сессии, без полного compositor/GPU-учёта, мобильного устройства и OBS.

Остаётся: новый край нужно оценить художественно; возможный эффект «вырезанного» силуэта не считается исправленным до оценки владельца. Холодная сборка блокирует UI на слабом CPU; её следует готовить вне основного потока и ограничить память кэша при интеграции. В `PencilScene` по-прежнему нет полноценного `brushId`, анимация рисования надписи не использует новый художественный рендер, `.myb` и акварель не реализованы.

## Handoff 2026-09-23: первый статичный образец кистей (UX-TASK-043)

Сделано: в `packages/pencil-engine/src/gbr.ts` добавлен валидируемый импорт `.gbr` v2 (маска/RGBA); в `src/brush-study.ts` — отдельное нанесение волокон по направленным кривым и печать импортированного наконечника. В `demo/study.html`, `study.ts`, `study-scene.ts`, `study.css` добавлено сравнение с исходной картинкой на 180/512/1000 px, смена трёх кистей тела/канистры и загрузка `.gbr`. Многовходовая сборка настроена в `vite.config.ts`. Снимки хранятся в `reference/study-compare-{180,512,1000}.png`; обновлены README, `docs/PENCIL_ENGINE.md`, `docs/PENCIL_ENGINE_BRUSHES.md`, `docs/PENCIL_ENGINE_VISUAL_REWORK.md`, `docs/tasks/ui-ux/UX-TASK-043.md`, `docs/DECISIONS.md`, `docs/CHANGELOG.md` и этот handoff.

Проверки: `npm run test -w @fanfuel/pencil-engine` — 8 PASS; `npm run typecheck -w @fanfuel/pencil-engine` и `npm run build:demo -w @fanfuel/pencil-engine` — PASS; точечный ESLint — 0 ошибок (конфиг Vite проигнорирован текущей ESLint-конфигурацией). Headless Chrome: все три размера без page errors, переключение кисти меняет пиксели, возврат к прежней кисти воспроизводит тот же кадр, загруженный `.gbr` меняет изображение. На настольном Chrome встроенные кисти заняли примерно 209/214/428 мс при 180/512/1000 px; импортированный наконечник на 512 px — 1,45 с. Это единичные локальные замеры, не бюджет анимации.

Ограничения: при визуальном просмотре новый рисунок всё ещё слишком регулярный и мало похож на исходную фактуру. Профиль нажима, карта тона, бумажный микрорельеф, полноценный `brushId` в `PencilScene`, `.myb`, слабые устройства и OBS не проверены. Художественное принятие не получено. Дальше по `UX-TASK-043`: дорабатывать направление/плотность/локальный тон на статичном фрагменте и подтверждать сходство владельцем, затем интегрировать пресеты в формат сцены и оптимизировать для анимации.

## Handoff 2026-09-23: сменные кисти для карандашного движка (UX-TASK-043)

По запросу владельца изучены официальные форматы и движки кистей. Создан `docs/PENCIL_ENGINE_BRUSHES.md`: выбран независимый контракт траектории/пресета, `.gbr` v2 для наконечника и `.myb`/libmypaint как кандидат динамического импорта; зафиксированы первые три карандашных материала, отдельный будущий акварельный backend, ограничения совместимости и лицензий. Обновлены `docs/PENCIL_ENGINE.md`, `docs/PENCIL_ENGINE_VISUAL_REWORK.md`, `docs/tasks/ui-ux/UX-TASK-043.md`, `docs/DECISIONS.md`, `docs/CHANGELOG.md` и этот handoff.

Проверка: официальные спецификации GIMP, документация Krita/Procreate/Adobe и репозитории MyPaint сверены; локальные ссылки и diff проверены. Код не менялся, автоматические тесты не запускались. Риск: новый рендер, импорт кистей и точность переноса `.myb` ещё не проверены, версия `0.1.0` визуально отклонена. Следующая задача `UX-TASK-043`: статичный сравнительный лист, валидируемый импорт `.gbr` и три переключаемых карандашных пресета до анимации.

Документ для передачи работы между агентами.

## Handoff 2026-09-23: отклонение визуального результата UX-TASK-043

Владелец оценил демо `0.1.0` как не похожее на референс. Причина: текущая равномерная заливка и случайные короткие штрихи не передают направленную многослойную работу карандашом. Реализация не менялась; статус задачи установлен `VISUAL_REWORK_REQUIRED`. Новый технический план и художественные критерии записаны в `docs/PENCIL_ENGINE_VISUAL_REWORK.md`. Исходное изображение сохранено в `packages/pencil-engine/reference/oily-original.png`; через встроенный imagegen созданы два прозрачных диагностических образца тела и канистры, не являющиеся точными слоями исходника. Их происхождение описано в `packages/pencil-engine/reference/README.md`. Третий образец контура отклонён и в проект не добавлен. Следующий шаг — один статичный сравнительный фрагмент тела/канистры/контура с авторскими направлениями штрихов и моделью нажима/бумаги; только после принятия вида — анимация и оптимизация. Проверка: alpha-прозрачность и размеры образцов проверены; код и тесты не менялись.

## Handoff 2026-09-23: UX-TASK-043, карандашный движок

Сделано: добавлен независимый `packages/pencil-engine` с векторными путями, адаптивной графитной фактурой, стабильным написанием, раскладкой авторских глифов, покадровым `boil`, вложенными группами и векторными кадрами спрайта. Демо по мотивам предоставленного рисунка и подробный авторинг/API/roadmap находятся в `packages/pencil-engine/demo` и `docs/PENCIL_ENGINE.md`. Обновлены `package-lock.json`, `docs/tasks/ui-ux/UX-TASK-043.md`, `docs/TASKS.md`, `docs/ARCHITECTURE.md`, `docs/DECISIONS.md`, `docs/CHANGELOG.md`. Production-страницы, API и платежи не менялись.

Проверки: `npm run test -w @fanfuel/pencil-engine` — 5 PASS; `npm run typecheck -w @fanfuel/pencil-engine`, `npm run build:demo -w @fanfuel/pencil-engine` и точечный ESLint — PASS; headless Chrome открыл обе сцены без page errors, получены снимки при 180/512 px. Локальный benchmark `draw()` после прогрева при DPR 2: 0,83/1,43/1,72 мс для 180/512/750 px на рабочем ПК. Это не измерение слабого телефона, compositor или пачки одновременных стикеров.

Не проверено: художественное сходство на нескольких эталонах, мобильный CPU/память, OBS, автоматическая конвертация текста и безопасный импорт SVG. Дальше: визуальная калибровка с художником, конвертер глифов/SVG, профилирование и только затем решение об интеграции с Ойли по `UX-TASK-038`.

## Handoff 2026-09-23: демонстрационные товары

В dev seed вынесен единый SQL сценарий `infra/scripts/seed-demo-catalog.sql`; PowerShell и shell обёртки больше не создают прежние англоязычные примеры. Сценарий безопасно повторяется: он upsert-ит три фиктивных профиля без рейтингов, добавляет шесть русскоязычных demo listings с настроенными тестовыми долями и публичными превью в `product_media`. Обложки — шесть собственных SVG по `/products/demo/`, 960×600, без внешних запросов и чужих игровых скриншотов. В локальной БД dev API данные добавлены; API вернул шесть товаров, все шесть обложек отдаются web на localhost. Публичные и production данные не менялись. Файлы и назначение зафиксированы в `docs/DESIGN_ASSETS.md`.

## Handoff 2026-09-22: UX-TASK-042

Сделано: заменены четыре однотипные рекламные карточки редакционным указателем восьми разделов; добавлены игры, программы и материалы для игр через миграцию 000011 и ru/en-словарь. Вкладки гостевых ролей получили рисуемый карандашный SVG-контур. Ойли показывается в первом экране как один actor, перемещается между якорями покупателя/автора/продавца/шага, сохраняет реакции по нажатию и скрытие через sessionStorage; из empty/error и нижнего блока копии удалены. Новые пользовательские маршруты и endpoints не вводились. Менялись `marketplace-experience.tsx`, `role-presentation.tsx`, новый `oily-stage.tsx`, `editorial.css`, i18n, миграция, browser smoke, соответствующие docs/ADR.

Проверки: `npm run lint`, `npm run typecheck -w @fanfuel/web`, `npm run build -w @fanfuel/web`, `npm run test:experience -w @fanfuel/web`, `go test ./...` с изолированной PostgreSQL — PASS. На localhost:3000 и API:8080 проверены навигация/категории; миграция 000011 применена к локальной dev БД. Playwright: 320/390/768/1440, light/dark, ru/en, ARIA/клавиатура, reduced motion/Save-Data, скриншоты `%TEMP%/fanfuel-experience-qa/`. Исследовательские источники и их пределы указаны в UX-TASK-042; порядок разделов пока гипотеза без продажной статистики.

Риски и дальше: собственные данные о спросе и Core Web Vitals на физических слабых телефонах ещё не собраны. Широкая категория не разрешает ключи, gift или аккаунты. Для rollback миграции после появления товаров нужна переклассификация. Следующее — product review владельца, затем реакции Ойли/игра отдельной задачей UX-TASK-038/039.

## Handoff 2026-09-22: живой marketplace, UX-TASK-040

### Сделано

- Утверждённое владельцем направление перенесено в web: графит/тёплая поверхность/лайм #cafa39, знак канистры-F, системная типографика, векторные рисунки и карандашные акценты.
- Главная: три доступные с клавиатуры вкладки только гостям; после входа — marketplace. В шапке поиск, каталог, избранное, корзина и единый вход/регистрация; презентации автора/продавца вынесены в footer и отдельные индексируемые страницы.
- /for-streamers и /for-sellers: полноценные объяснения, интерактивные шаги, FAQ, CTA в существующие сценарии; будущие партнёрские механики отделены от работающего MVP.
- Каталоговые заглушки заменены выдачей существующего API: query/category/sort, limit/offset, «Показать ещё», retry и отмена устаревших ответов. /marketplace/categories показывает ту же актуальную торговую поверхность.
- Избранное/корзина: локальные списки ID, гостевой доступ, межвкладочное обновление, ограничение 100 товаров, данные перепроверяются API в drawer. Checkout отдельного товара использует прежний маршрут; мультизаказ не добавлен.
- SVG-Ойли с репликами по нажатию и скрытием. Декоративная зарисовка однократно при открытии пути за сессию, без replay на reload; reduced-motion/Save-Data отключают движение. Общие кнопки/фокус/pressed состояния обновлены на web.
- Размеры skeleton, карточек и пустого состояния согласованы; внешние шрифты, растровые брендовые ассеты и animation runtime не добавлены. Сетевой сбой больше не удаляет токен в шапке.

### Изменённые области

`apps/web/src/app/experience.css`, layout, marketplace/*, for-streamers, for-sellers; новые components/experience-provider, experience-footer, marketplace-experience, role-presentation, market-lists, brand-art, oily; app-chrome, lib/api, lib/i18n, use-auth-session. Добавлены ru/en experience dictionaries и общие поисковые placeholder. Playwright — только devDependency; smoke и команда test:experience находятся в apps/web. Обновлены roadmap, актуальный UI-контракт, tasks, ADR, audit и этот handoff. Ранее имевшиеся изменения авторизации/Go/документов сохранены.

### Проверки

- PASS: `npm run lint`.
- PASS: `npm run typecheck` для всех workspaces.
- PASS: `npm run build -w @fanfuel/web` (Next 16.3.3, production).
- PASS: `npm run test:experience -w @fanfuel/web` против production на 127.0.0.1:3002; API-подмены существуют только в тесте.
- Пройдены guest/member/session error, вкладки с клавиатуры, сохранение после reload, корзина и checkout URL, поиск/серверная пагинация, ru/en, 320/390/768/1440 без horizontal overflow, light/dark, reduced-motion, Save-Data, CPU ×4, недоступное browser storage. Геометрия skeleton/товаров/empty проверяется bounding boxes. Страницы ролей видимы с отключённым JS и содержат index/follow metadata.
- Скриншоты визуально просмотрены: desktop, mobile, dark, авторская страница. Артефакты вне git: `%TEMP%/fanfuel-experience-qa/`.

### Ограничения и следующие шаги

API:8080 и БД локально не запущены, Docker недоступен. Поэтому полный живой checkout, backend permissions и данные БД в этом прогоне не проверены; frontend показывает честную ошибку при недоступном API. Реальные платежи/выплаты не подключались.

В Product API нет медиа: показаны подписанные иллюстрации категорий, не вымышленные обложки. Корзина/избранное не синхронизируются между устройствами. Ойли — SVG-пилот, не полноценная игра с ходьбой/эволюцией. Core Web Vitals p75 и физический слабый телефон требуют отдельного замера после подключения backend.

Следующие шаги UX-TASK-040: review владельца, интеграционный прогон с dev stack и реальные performance-замеры. UX-TASK-038: character bible и пилот событий; UX-TASK-039/World остаётся FUTURE без наказаний за отсутствие и давления на покупки.

## Handoff 2026-09-21: правила аккаунтов, ключей и пополнений в MVP

### Что сделано

- Вместо общего вечного запрета введён category-by-category подход: аккаунты, подписки, ключи, пополнения и внутриигровые товары по умолчанию `restricted`, но отдельная подкатегория может быть разрешена после legal/payment/provider review.
- Зафиксировано различие между законным владением и правом передачи. Украденные, взломанные, shared, promo-abuse и содержащие чужие данные аккаунты запрещены.
- Добавлены требования к provenance, platform ToS, privacy cleanup, безопасному handover без хранения секретов, anti-fraud и refund/dispute policy.
- ADR-0025 фиксирует решение. По актуальным условиям Steam и OpenAI соответствующие личные аккаунты не допускаются; Google/Gemini и Perplexity не считаются разрешёнными без отдельной проверки.

### Изменённые документы

- `docs/PRODUCT_SPEC.md`, `docs/roadmap/principles.md`, `docs/PAYMENTS.md`, `docs/LEGAL_REVIEW.md`, `docs/DOMAIN_MODEL.md`, `docs/SECURITY.md`, `docs/DO_NOT_BUILD_YET.md`.
- `docs/DECISIONS.md`, `docs/tasks/v0.3/FF-0301.md`, `docs/tasks/v1.0/FF-1004.md`, `docs/tasks/v1.0/FF-1005.md`, `docs/CHANGELOG.md`, `docs/HANDOFF.md`.

### Проверки и риски

- Выполнены поиск противоречащих правил по документации и проверка diff. Код, API, БД и i18n не менялись; автоматические тесты не запускались.
- `LEGAL_REVIEW_REQUIRED` и `PAYMENT_REVIEW_REQUIRED` сохраняются. Эта правка не открывает restricted категории и не является юридическим заключением.
- Перед привлечением продавцов конкретных аккаунтов или пополнений нужно сначала зафиксировать platform-specific решение в FF-1004/ADR.

## Handoff 2026-09-17: DNS-подготовка исходящей почты FanFuel

- В панели REG.RU для `fanfuel.ru` добавлены TXT-записи `@ = v=spf1 ip4:194.37.1.112 -all` и `_dmarc = v=DMARC1; p=none; adkim=s; aspf=s`.
- Записи сохранены панелью с подтверждением «Внесены изменения зоны». Сразу после сохранения авторитетные `ns1.reg.ru`/`ns2.reg.ru` ещё отдавали прежнюю зону; требуется повторная проверка после DNS propagation.
- DKIM пока не добавлен: публичный ключ должен быть сгенерирован существующим сервером `mail.danechka.com` после подключения домена `fanfuel.ru`.
- MX не добавлялся, поскольку текущий scope — только исходящая почта. Существующие A-записи сайта не изменялись.
...
- `npx prettier --check` для изменённых auth/i18n/task-файлов — успешно.
...
## Восстановление VM и автозапуск — 2026-09-17, FF-0009/FF-0010

Запущены три актуальные VirtualBox VM: `FanFuel`, `nginx-edge-prod2` и `librechat-ubuntu-server`. Устаревшие edge-клоны не запускались: они имеют одинаковый MAC-адрес и не должны работать одновременно. Для пользователя Windows `remot` создано задание планировщика `FanFuel - Start VirtualBox VMs`: через 15 секунд после входа оно без окон проверяет состояние всех трёх VM и запускает отсутствующие в headless-режиме. Тестовый запуск задания завершился с кодом `0`. Триггер непосредственно при старте Windows не создан: текущий сеанс не имеет административных прав, Windows вернула `Access denied`.

VM `FanFuel` была частично зависшей: консоль фиксировала `watchdog: BUG: soft lockup`, SSH не отвечал, публичный API возвращал `502`. Штатный ACPI-сигнал не был обработан; после reset SSH восстановился. PostgreSQL оказался остановлен с `Exited (255)`, выполнил штатное WAL recovery и сообщил `database system is ready to accept connections`; переинициализация и изменение данных не выполнялись. У единственного контейнера PostgreSQL фактическая restart policy была `no`; для всех девяти контейнеров runtime-политика приведена к `unless-stopped`, после чего API и WS перезапущены.

Проверки: все три VM имеют состояние `running`; Windows-задача имеет `LastTaskResult = 0`; edge nginx в Alpine имеет статус `started` и включён в runlevel `default`; PostgreSQL, Redis, MinIO и worker healthy; внутренние `api`, `ws`, `worker`, `web` health/smoke отвечают `200`; публичные `https://fanfuel.ru/`, `https://fanfuel.ru/api/v1/categories`, `https://danechka.com/` и локальный LibreChat `http://127.0.0.1:3080/` отвечают `200`.

Файлы проекта не менялись, кроме этой записи в `docs/HANDOFF.md`; существующие незакоммиченные изменения сохранены. Риски: первопричина kernel soft lockup не устранена; нужен отдельный аудит VirtualBox/ядра/containerd. До входа пользователя Windows VM автоматически не стартуют; для запуска до экрана входа требуется настроить системное задание или службу из административного сеанса.

## Handoff 2026-09-20: переработка плана marketplace, маскота и FanFuel World

Задача: переработать план разработки после изменения структуры главной и добавить последующие этапы живого интерфейса, маскота и `world.fanfuel.ru`.

Что сделано: добавлен `UX-TASK-036` как P0-контракт главной: `/` и `/marketplace` остаются одной marketplace-first поверхностью, обычный покупатель всегда выбран по умолчанию, автор и продавец доступны вторичными вкладками, а общий каталог не ограничивается creator/OBS товарами. PAGE-HOME, PAGE-MARKETPLACE и buyer home/search flows переведены в `NEEDS_REWORK`; UX-TASK-029/030 помечены как исторические. В `docs/DESIGN_AUDIT.md` зафиксировано расхождение между текущей реализацией и целевой структурой.

Дальнейший порядок: `UX-TASK-037` — прототипы и выбор живого визуального языка вместо generic corporate UI; `UX-TASK-038` — 3–5 концепций, character bible, reaction matrix и безопасный UI-пилот контекстного маскота; `UX-TASK-039` — только discovery FanFuel World. `PAGE-WORLD-HOME` добавлена как FUTURE и `DO_NOT_BUILD_YET`; production route/app, account progress, валюта, loot boxes, NFT, платные случайные награды и привязка прогресса к сумме покупок/донатов не разрешены.

Изменённые файлы: `README.md`, `ROADMAP.md`, `docs/PRODUCT_SPEC.md`, `docs/INFORMATION_ARCHITECTURE.md`, `docs/UX_PLAN.md`, UI/UX specs/tracker/flows/rules/audit, design system/theming, roadmap/current focus/future, task index и новые файлы `docs/roadmap/experience-brand-world.md`, `UX-TASK-036`–`UX-TASK-039`.

Проверки: `rg`-сверка статусов и устаревших требований к hero; проверка всех локальных Markdown-ссылок; `git diff --check`; `npx prettier --check` для изменённых плановых документов, кроме уже содержавшего чужие изменения `docs/HANDOFF.md`. Product/browser/code tests не запускались, потому что production-код не менялся.

Риски и следующий шаг: текущая главная ещё не подтверждена product review и остаётся `NEEDS_REWORK`. Следующая реализационная задача — только UX-TASK-036 с regression QA на `ru/en`, light/dark, reduced motion, 320/390/768/1440, данных и empty/error states. Существующие незакоммиченные auth/API/infra изменения и ранее добавленные handoff-записи не изменялись и не откатывались.

## Диагностика недоступности сайта — 2026-09-22, FF-0009/FF-0010

Что проверено: `https://fanfuel.ru/` и публичный API не отвечают в течение 12–15 секунд; DNS и TCP 80/443 доступны. Задание Windows `FanFuel - Start VirtualBox VMs` настроено только на вход пользователя `remot` с задержкой 15 секунд, последний штатный запуск был 18.09.2026; ручной запуск 22.09.2026 завершился кодом `0`. Задание проверяет только состояние VirtualBox `running` и не обнаруживает зависшую гостевую ОС. VM `FanFuel` и `nginx-edge-prod2` числятся запущенными с 18.09.2026. Edge доступен по SSH, nginx запущен и направляет `fanfuel.ru` на VM приложения. VM приложения не отвечает по SSH (включая NAT-порт 2223) и LAN; её консоль показывает `watchdog: BUG: soft lockup`. Штатный ACPI power button отправлен, но за 20 секунд VM не выключилась и SSH не восстановился.

Файлы: изменён только `docs/HANDOFF.md`; ранее существовавшие незакоммиченные изменения не тронуты. Проверки: статус планировщика, VirtualBox, SSH, TCP, HTTP, журналы edge и снимок консоли VM. Код и контейнеры не менялись; тесты проекта не запускались, поскольку гостевая ОС недоступна.

Риски и следующие шаги: для восстановления VM требуется принудительный reset после подтверждения пользователя, поскольку внутри работает PostgreSQL. После запуска проверить WAL recovery, контейнеры и публичные HTTP/API, затем отдельно исследовать причину повторяющегося kernel soft lockup. Автозапуск при старте Windows потребует повышенных прав; текущий процесс имеет ограниченный токен администратора.

## Восстановление и автозапуск VM — 2026-09-22, FF-0009/FF-0010

По разрешению пользователя выполнен принудительный reset только VM FanFuel; edge и другие VM не перезапускались. SSH восстановился. PostgreSQL выполнил автоматическое WAL recovery и перешёл в состояние healthy; девять контейнеров поднялись, api, ws, worker, Redis, MinIO и PostgreSQL healthy. Внутренние /healthz и web, публичные https://fanfuel.ru/, /api/v1/categories, /auth/login ответили 200.

Причина повторяемости: в предыдущих загрузках 15, 17 и 18 сентября видно одинаковую цепочку clocksource: Long readout interval → RCU stall → watchdog: BUG: soft lockup на нескольких CPU, включая системные процессы и containerd-shim. На хосте HypervisorPresent=True; журнал VirtualBox сообщает AMD-V is not available, переход на NEM и невозможность RealTSCOffset. Это сильное свидетельство конфликта Hyper-V/VirtualBox и задержек виртуального времени, но контрольного запуска без Hyper-V ещё не было. В госте после восстановления свободно 3,1 GiB RAM и 26 GiB диска; в предыдущем kernel log не обнаружены OOM и ошибки файловой системы/ввода-вывода. Официальные инструкции Oracle и Microsoft предупреждают о проблемах совместной работы: https://docs.oracle.com/en/virtualization/virtualbox/7.2/user/Troubleshooting.html и https://learn.microsoft.com/en-us/troubleshoot/windows-client/application-management/virtualization-apps-not-work-with-hyper-v.

На Windows создан C:\ProgramData\FanFuel\start-vms.ps1 и задание FanFuel - Start VirtualBox VMs at Boot с триггером AtStartup через 30 секунд, аккаунтом remot и S4U. Прежний триггер при входе FanFuel - Start VirtualBox VMs переведён на тот же скрипт. Скрипт проверяет состояния трёх актуальных VM, запускает только выключенные и пишет C:\ProgramData\FanFuel\vm-startup.log. Для журнала настроены права записи аккаунта remot. В режиме S4U путь под LocalAppData оказался недоступен, поэтому скрипт хранится в ProgramData. Ручной запуск обоих заданий завершился с LastTaskResult=0; проверка с выключенной VM и реальная перезагрузка Windows не проводились, чтобы не прерывать работающие сервисы.

Файлы репозитория: docs/HANDOFF.md, docs/CHANGELOG.md, docs/DECISIONS.md; production-код не менялся. Вне репозитория: скрипт и два задания Windows. Проверки: SSH, Docker health, PostgreSQL WAL log, HTTP 200, Task Scheduler S4U и интерактивный запуск, анализ журналов предыдущих загрузок. Тесты кода не запускались, поскольку код не менялся. Следующий шаг: после оценки зависимости Windows от Hyper-V и окна обслуживания проверить VM без Hyper-V либо перенести публичный сервис на постоянно работающий Linux-хост. До этого автозапуск не предотвращает зависание уже работающей VM; автоматический reset с PostgreSQL не включён.

Дополнительное наблюдение через 15 минут после reset: в текущей загрузке снова появились clocksource Long readout interval, RCU stall и soft lockup CPU#3 на 228 секунд. Публичная главная в этот момент ещё отвечала 200. Симптом повторился без длительной работы приложения; риск нового полного зависания остаётся высоким. На Windows включён VirtualMachinePlatform, WSL не установлен, VirtualizationBasedSecurityStatus=2; поэтому отключение гипервизора затронет текущий режим безопасности Windows и требует отдельного разрешения на изменение boot-конфигурации и перезагрузку всего ПК.

Уточнение по хосту: установлена Windows 11 Home. Наличие работающего гипервизора Windows не означает доступность роли Hyper-V для управления обычными VM: Microsoft не поддерживает эту роль в Home. Поэтому перенос FanFuel VM в Hyper-V потребовал бы обновления Windows до Pro и отдельной миграции. Пока этот ПК остаётся основным сервером, ближайший проверяемый вариант — однократная загрузка Windows без гипервизора при сохранении текущей конфигурации по умолчанию, затем проверка VirtualBox на прямой AMD-V, непрерывности работы VM и публичного сайта. Перед изменением нужен контроль режима защиты/шифрования и согласованная перезагрузка всех VM и хоста. Источник: https://learn.microsoft.com/en-us/windows-server/virtualization/hyper-v/get-started/install-hyper-v.

Предварительная проверка хоста с повышенными правами: BCD содержит hypervisorlaunchtype Auto; системный диск C: полностью расшифрован, BitLocker protection отключена; VBS status=2, SecurityServicesRunning=0. Изменения boot-конфигурации не выполнялись. Результат read-only проверки сохранён вне репозитория в C:\ProgramData\FanFuel\host-preflight.txt. Однократная загрузка без гипервизора всё равно временно меняет режим VBS и требует перезагрузки ПК; штатно остановить VM и переключить bootsequence можно только после разрешения пользователя.

## Однократная проверка VirtualBox без гипервизора Windows — 2026-09-22, FF-0009/FF-0010

Пользователь разрешил перезагрузку всего ПК. Подготовлена отдельная запись BCD `Windows 11 - VirtualBox AMD-V test` с `hypervisorlaunchtype off`; обычная запись Windows и её `Auto` сохранены. Резервная копия BCD: `C:\ProgramData\FanFuel\bcd-before-vbox-test-20260922-192221.bak`. Перед перезагрузкой нужно установить однократный `bootsequence` на GUID из `C:\ProgramData\FanFuel\one-time-bcd-guid.txt`. Подготовка записи сама по себе ещё не перезагрузила ПК.

Внутри FanFuel сделан согласованный дамп PostgreSQL `/home/fanfuel/backups/pre-host-test-20260922.sql.gz`; копия `D:\VirtualBox VMs\FanFuel\backups\pre-host-test-20260922.sql.gz`, SHA256 совпал. Задание `FanFuel - Verify after Windows Boot` через 3 минуты после запуска записывает состояние гипервизора, четырёх VM, сайта/API и NEM в `C:\ProgramData\FanFuel\post-boot-check.log`. При нынешнем Hyper-V ручной пробный запуск ожидаемо дал код 1. Реально проверена задача автозапуска: выключенная edge VM запущена через Task Scheduler, `LastTaskResult=0`, публичный сайт восстановился до HTTP 200 после загрузки edge. Скрипт автозапуска включает FanFuel, nginx-edge-prod2, librechat-ubuntu-server и InsideApiVezde.

Перед перезагрузкой две дополнительные VM (`librechat-ubuntu-server`, `InsideApiVezde`) не отреагировали на ACPI power button; оба проброшенных SSH-порта принимали TCP, но не выдавали SSH-приветствие, консоль InsideApiVezde показывала трассировку ядра. Это отдельное свидетельство повторяемости проблемы на нескольких VM. FanFuel и edge остановлены по SSH командой `sudo -n poweroff`; состояния VM нужно проверить перед самой перезагрузкой. Если дополнительные VM не завершатся штатно, выключение хоста прервёт их принудительно, так как пользователь разрешил перезагрузку всего ПК.

После загрузки проверить `post-boot-check.log`, `HypervisorPresent=False`, отсутствие NEM в новом VBox.log, доступность сайта и API, журналы ядра каждой VM, работу задания AtStartup и сохранение стандартной записи BCD как загрузки по умолчанию. Одна успешная загрузка подтвердит режим AMD-V и автозапуск, но устойчивость нужно оценить по длительной работе, поскольку прежние soft lockup повторялись через часы/дни. Тесты кода не запускались: production-код не менялся.

Итог попытки 19:32 MSK: Windows показала UAC при установке однократного `bootsequence`, затем вернула «Операция была отменена пользователем». Файл `one-time-boot-confirmation.txt` не создан; перезагрузка хоста и контрольный запуск без гипервизора **не выполнялись**. Повторная попытка повышения прав не предпринималась. Все четыре VM подняты через существующее задание автозапуска (`LastTaskResult=0`); FanFuel и публичный API снова отвечают HTTP 200, PostgreSQL и остальные сервисные контейнеры healthy. LibreChat после перезапуска выдаёт SSH host key, InsideApiVezde пока не выдаёт SSH-приветствие и требует отдельной проверки после загрузки. Для продолжения теста нужно подтвердить системный UAC, затем повторить остановку VM и однократный выбор тестовой записи. Пока Windows по-прежнему загружена с гипервизором, риск повторных зависаний сохраняется.

Продолжение 19:34 MSK: пользователь сообщил, что не видел первый UAC. Повторный запуск с видимым окном завершился успешно; `C:\ProgramData\FanFuel\one-time-boot-confirmation.txt` подтвердил `default {current}`, `bootsequence {439c7438-a978-11f1-9581-f2f38e86261d}`, `hypervisorlaunchtype Auto` у обычной записи и `Off` у тестовой. Повторно остановлены FanFuel и edge через SSH, LibreChat через ACPI; InsideApiVezde после повторного зависания пришлось выключить через VirtualBox `poweroff`. Все четыре VM выключены перед согласованной перезагрузкой. Проверка результата будет в `post-boot-check.log` после загрузки Windows.

Результат после перезагрузки: 19:40 MSK задание `FanFuel - Verify after Windows Boot` завершилось с кодом 0. `HypervisorPresent=False`, в новом VBox.log FanFuel нет NEM fallback, есть `HM: Enabled SVM` и переключение TSC в `RealTSCOffset`. Задание автозапуска в 19:37 подняло все четыре VM и завершилось с кодом 0. Главная и API ответили HTTP 200, девять контейнеров FanFuel работают, PostgreSQL и основные сервисы healthy. Через 17 минут работы в журнале текущей загрузки FanFuel не обнаружены `clocksource Long readout`, `RCU stall`, `soft lockup`. Этот интервал недостаточен для доказательства устранения повторяющегося сбоя. InsideApiVezde по-прежнему не выдаёт SSH-приветствие после перезапуска уже без гипервизора Windows; её неисправность требует отдельной диагностики. Тестовая запись BCD выбрана только на эту загрузку: следующая обычная перезагрузка вернёт запись `{current}` с `hypervisorlaunchtype Auto`, если не принять отдельное решение о постоянной конфигурации.

Уточнение InsideApiVezde: снимок консоли после прямой AMD-V показывает штатный `inside-api login:`; гостевая ОС загрузилась. SSH-порт 2224 по-прежнему не выдаёт приветствие, поэтому требуется диагностика SSH/сетевого стека именно этой VM. Это не доказывает, что её ядро сейчас зависло.

## UX-TASK-041 — дизайн и commerce foundation, 22.09.2026

Статус: IMPLEMENTED_LOCAL, review владельца ожидается. Реальных платежей, публикации в production и коммита не выполнялось. Существовавшие изменения auth/инфраструктуры/документации не откатывались.

### Что сделано

- Сопоставлены исходные изображения с сайтом; исправлены типографика/плотность главной, карточки и empty state. Карандашная фактура Ойли — 48 KB alpha WebP; рукописный текст — локальный Caveat. Golos и лицензии поставляются локально. Темы, i18n, reduced motion, SaveData и гостевые вкладки сохранены.
- Добавлен /studio: live preview, свои баннер/ава через signed upload, presets, заголовок/описание, grid/list и порядок блоков, донат справа/блоком/скрыт, поиск и выбор предложений, отдельное включение promo/storefront, реальные mock-данные статистики.
- Seller editor /seller/products/[id] и новый товар требуют identity и две ставки перед модерацией; варианты и отчисления показаны с примерами сумм и объяснением ценовых сигналов. Собственные черновики получают аналитику через owner-only endpoint.
- Marketplace search: exact variants, тип получения, платформа, регион, русский FTS и опечатки, grouped cheapest offer; в Studio каждый продавец виден отдельно. Product показывает строгие аналоги, формат и server quote с именем/суммой поддержки. Cart/saved сохраняют источник до checkout.
- Поддержка оплачивается продавцом, buyer price неизменен. Конфликт двух авторов требует выбора одного. Order сохраняет snapshot; fingerprint обнаруживает изменение условий. Buyer-scoped idempotency и advisory lock выдерживают конкурентные повторы.
- Библиотека алгоритмов для повторного использования: identity, money/refunds, median/MAD, confidence/Wilson, Bayesian rating, diversification, inventory reservations, complaint review. Карта реально подключённых и подготовленных функций — MARKETPLACE_ALGORITHMS.md.

### Файлы

Frontend: apps/web/src/app/{editorial,commerce}.css, layout.tsx, страницы marketplace/product/checkout/creator/seller/studio; components/{brand-art,marketplace-experience,experience-provider,market-lists,product-cover,commerce-shared,creator-commerce-editor,creator-media,seller-commerce-editor,storefront,app-chrome}; lib/{api,commerce}.ts; public/{brand,fonts}; tests/{experience-smoke,commerce-smoke}.cjs. Словари ru/en commerce/experience/errors, packages/i18n/src/messages.ts; совместимое поле Product.cover_url в packages/types.

Backend: services/api/internal/commerce/*; app/commerce_{types,store,handlers,integration_test}.go; app/media{,_test}.go; marketplace_{types,store,handlers}.go; app.go/errors.go. Миграции 000008, 000009, 000010 up/down. eslint игнорирует .tmp runtime-инструменты.

Документация: MARKETPLACE_ALGORITHMS.md, DESIGN_ASSETS.md, UX-TASK-041; PAYMENTS/DOMAIN_MODEL/API_PLAN/DECISIONS; specs/tracker/status/tasks/flows/page map/audit/design system; ROADMAP/current-focus/active-focus; HANDOFF/CHANGELOG.

### Проверки

- npm run typecheck — все workspaces, успешно.
- npm run lint — успешно, без ошибок/предупреждений ESLint.
- npm run build -w @fanfuel/web — production build успешно.
- npm run test:experience -w @fanfuel/web — успешно: guest/member/error, keyboard tabs, saved/cart, поиск/пагинация, ru/en, light/dark, 320/390/768/1440, loading/empty/error, reload/reduced motion/SaveData, 4× CPU, недоступное storage.
- npm run test:commerce -w @fanfuel/web — успешно: оформление/порядок/сохранение, независимые каналы, недостаточная ценовая выборка, responsive Studio, сохранение черновика при 409, cart attribution, выбор одного автора и seller rate validation.
- gofmt и go test ./... из services с FANFUEL_TEST_DATABASE_URL — успешно. Использован официальный Go 1.26.8 из .tmp/tools/go, поскольку Go отсутствовал в PATH. Интеграция на отдельной PostgreSQL 16.15, БД fanfuel_commerce_test (127.0.0.1:55439), отдельные удаляемые тестовые схемы. Полный набор migrations up и 000008–000010 down/up проверен.
- Проверены границы вариантов/неизвестные атрибуты, переполнение/сохранение копеек при возврате, валюты/выбросы/cold start, search typo/grouping, ownership, неизменность snapshot, конфликт получателя, 6 одновременных повторов заказа при pool=2, upload signature/replay/purpose/visibility. Обложки не раскрывают private media.
- Реальный localhost:3000 проверен через in-app browser; скриншоты fixtures отдельно в системном Temp/fanfuel-experience-qa и fanfuel-commerce-qa. При визуальной проверке поправлены чрезмерный трекинг заголовков и тесная карточка единственного товара в мобильной витрине.

### Ограничения и дальнейшие задачи

- Локальный mock API на 127.0.0.1:8080 работает с отдельной тестовой БД без фиктивных публичных товаров. Для него используется игнорируемый services/api/cmd/tmp; runtime-инструменты/данные лежат в .tmp, не в поставке проекта. Рабочая БД пользователя не мигрировалась. Docker отсутствовал, поэтому проверки выполнены напрямую с PostgreSQL.
- Production payment/payout/ledger/refund и S3/R2 не проверялись и не подключены. MediaStorage имеет рабочий local adapter только вне production. Следующий FF-0105: durable adapter, GC/quota, seller product-media upload и проверка восстановления после сбоев.
- Источники canonical IDs пока вводятся продавцом и проверяются модерацией; импорт, aliases, admin merge/split и отдельные регионы activation/use — следующая задача перед открытием keys/accounts. Рискованные категории по-прежнему LEGAL_REVIEW_REQUIRED/PAYMENT_REVIEW_REQUIRED.
- Ориентир цены ограничен предложениями FanFuel, не внешним рынком. Пока нет telemetry и достаточной выборки, конверсии/рекомендации/anti-fraud функции не включаются автоматически. Нужны privacy-safe события, калибровка и нагрузочные тесты; реальные UX/конверсионные эффекты пока не измерялись.
- Списки локальны устройству. Полная игра Ойли, подписки/bot integration, production checkout остаются отдельными задачами; характер Ойли не привязан к расходам пользователя.
- Следующий шаг по UX-TASK-041: review владельца на живом сайте; статус не означает, что новый внешний вид уже принят пользователем.
...
## Публикация репозитория — 2026-09-24

Текущие изменения commerce, marketplace, Pencil Engine, миграций, инфраструктуры, i18n и документации подготовлены к синхронной публикации в GitHub и GitLab. Временный корневой скриншот `Изображение Codex 18 сент. 2026 г., 06_17_50.png` в коммит не включён.

Проверки: `npm run typecheck` — успешно; `npm run build` — успешно; `npm run lint` — не пройден из-за четырёх `no-undef` в `infra/scripts/playground-lan-proxy.mjs` для `process`, `URL` и `console`; `npm run test` и повторный тест `@fanfuel/pencil-engine` — 39 из 40 успешно, стабильно падает проверка «настройка мягкости меняет растяжение при одинаковом рывке» в `soft-body.test`; `go test ./...` не запущен, поскольку Go отсутствует в `PATH` на текущем Windows-хосте.

Риски и следующий шаг: исправить Node globals в ESLint-конфигурации или назначить окружение скрипту, разобраться с расчётом мягкости либо порогом соответствующего теста, затем повторить lint/tests и Go-тесты в окружении с установленным Go.
